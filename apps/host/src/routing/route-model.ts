...
export function createRouteModel(input: {
  ...
  function compile(route: ResolvedRoute): void {
    ...
    if (!done.has(id)) {
      ...
      else if (parent && parent.ownerPluginId !== route.ownerPluginId && !parent.contribution.acceptsChildren) fail(route, 'QUARANTINED', { code: 'EXTENSION_POINT_CLOSED', related: [identity(parent, 'route')] });
      else if (parent && parent.ownerPluginId !== route.ownerPluginId && !input.policy?.({ contributorId: route.ownerPluginId, ownerPluginId: parent.ownerPluginId, kind: 'route', targetId: parent.contribution.id })) fail(route, 'QUARANTINED', { code: 'POLICY_DENIED', related: [identity(parent, 'route')] });
      else {
  ...
  const items: ResolvedNavigation[] = [...input.navigation].sort((a, b) => (a.contribution.order ?? 1000) - (b.contribution.order ?? 1000) || stable(a, b)).map(item => ({ ...item, state: 'AVAILABLE', diagnostics: [], children: [] }));
  ...
  function compileNavigation(item: ResolvedNavigation): void {
    ...
    if (!navDone.has(id)) {
      ...
      else if (parent && parent.ownerPluginId !== item.ownerPluginId && !parent.contribution.acceptsChildren) navFail(item, 'QUARANTINED', { code: 'EXTENSION_POINT_CLOSED', related: [identity(parent, 'navigation')] });
      else if (parent && parent.ownerPluginId !== item.ownerPluginId && !input.policy?.({ contributorId: item.ownerPluginId, ownerPluginId: parent.ownerPluginId, kind: 'navigation', targetId: parent.contribution.id })) navFail(item, 'QUARANTINED', { code: 'POLICY_DENIED', related: [identity(parent, 'navigation')] });
      navDone.add(id);
    }
    navStack.pop();
  }
  ...
  return { routes, navigation, toRouteObjects, resolve, destination, listHostContributions };
}
