...
export default defineConfig((_env, argv) => {
  ...
  if (standalonePreview && !isDevelopment)
    throw Error('Standalone preview theme must not enter the integrated production artifact');
  ...
  return {
    ...
    plugins: [
      new rspack.CopyRspackPlugin({
        patterns: [
          {
            from: version === v1.version ? 'manifest.json' : 'manifest-v2.json',
            to: 'manifest.json',
          },
        ],
      }),
