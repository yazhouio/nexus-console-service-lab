...
export function App() {
  ...
  return (
    ...
    >
      ...
      <button className="kubeeye-button" disabled={bridgeState !== 'CONNECTED'} onClick={async () => {
        setCluster('Loading');
      ...
      <button className="kubeeye-button" disabled={bridgeState !== 'CONNECTED'} onClick={() => { void navigateToExampleNode().catch(error => setNavigationError(error.message)); }}>Open example node</button>
