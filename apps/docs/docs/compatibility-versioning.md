...
若要回退到旧 validator，应先在兼容 Host 清除所有已保存的新 schema 版本，再重新安装旧声明；仅切换 active version 仍会保留不可读的新版本。兼容测试覆盖该拒绝及恢复路径。生产构建默认启用 routing，目标环境验证流程见 [Host 路由实施与启用](maintainers/host-routing-implementation.md)。
