...
export function IndependentSurfacesFixture() {
  ...
  const route = runtime.contributions
    .listRoutes()
    .find((r) => r.contribution.id === 'kubeeye-overview-route');
  const overview = runtime.contributions
    .listRoutes()
    .find((r) => r.contribution.id === 'host-overview')?.contribution.target;
  const Overview = overview?.kind === 'builtin' ? (overview.render as ComponentType) : () => null;
  return (
    <>
      {route?.contribution.target.kind === 'sandbox-surface' && (
        <SurfaceMount
          pluginId={route.ownerPluginId}
          target={route.contribution.target}
          mountPointId={`route:${route.contribution.id}`}
          label="KubeEye Surface"
          testId="surface"
        />
      )}
      <Overview />
