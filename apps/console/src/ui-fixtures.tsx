...
export const uiFixtureInstallations = ['ui-a','ui-b','ui-c'].map(owner => validateRestrictedInstallRecord({
  manifest: { id: owner, version: '1.0.0', entry: `/plugins/${owner}/1.0.0/`, hostApi: 'kubesphere.console@1', requires: [UI_OVERLAY_CAPABILITY], provides: [], permissions: ['ui.overlay'], surfaces: [{ id: 'main' }, { id: 'dialog' }],
    ...
    contributions: { extensions: owner === 'ui-a' ? [] : [...(owner === 'ui-c' ? [{ kind: 'tab', id: 'node-chart', tabId: 'chart', label: 'Node chart', surfaceId: 'main', point: { ownerPluginId: 'cluster', id: 'node.tabs', contractMajor: 1 } }] : []), { id: `${owner}-card`, kind: 'surface', surfaceId: 'main', order: 0, point: { ownerPluginId: owner === 'ui-b' ? 'ui-a' : 'ui-b', id: owner === 'ui-b' ? 'details' : 'children', contractMajor: 1 } }] },
...
export function UiCompositionFixture() {
  ...
  return <><h1>UI composition acceptance</h1><button onClick={() => setShown(value => !value)}>Toggle A page</button>{shown && <SurfaceMount pluginId="ui-a" target={{ kind: 'sandbox-surface', surfaceId: 'main' }} mountPointId="ui-fixture-root" testId="ui-root" label="A page" autoMount />}<pre data-testid="ui-inspection">{JSON.stringify(snapshot)}</pre><pre data-testid="ui-instances">{JSON.stringify(adapter.listInstances().map(i => ({ ...i.identity, state: i.state.state, session: i.bridgeSession })))}</pre></>;
}


uiFixtureInstallations.push(validateRestrictedInstallRecord({
  manifest: { id: 'ui-action', version: '1.0.0', entry: '/plugins/ui-action/1.0.0/', hostApi: 'kubesphere.console@1', requires: ['routes.query@1'], provides: [], permissions: ['routes.query'], surfaces: [], actions: [{ id: 'check' }, { id: 'slow' }], contributions: { extensions: [
    { kind: 'action', id: 'check', actionId: 'check', label: 'Check node context', point: { ownerPluginId: 'cluster', id: 'node.actions', contractMajor: 1 }, expectedProfile: 'detail.actions@1', expectedRefContract: 'cluster.resource-ref@1' },
