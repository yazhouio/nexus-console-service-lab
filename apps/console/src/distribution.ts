...
const localDemo: PluginDefinition | undefined = NEXUS_REMOTE_DEMO ? undefined : require('./plugins/extension-demo').extensionDemo;
const demoCss: readonly string[] = NEXUS_REMOTE_DEMO ? [] : require('./plugins/extension-demo.css?artifact').css;
...
export const distribution: BrowserDistribution = {
  builtinCss: { ...(fixtures ? { [uiFixtureBuiltin.id]: localCss } : {}), [core.id]: coreCss, [cluster.id]: clusterCss, ...(localDemo ? { [localDemo.id]: demoCss } : {}),
  ...
  applicationLabel: 'Console', prepareBuiltins: NEXUS_BUILTIN_PINS.length ? federationBuiltins(NEXUS_BUILTIN_PINS) : undefined, builtins: [core, cluster, ...(localDemo ? [localDemo] : []), deployment, deploymentHpa, deploymentVpa, deploymentMonitoring, deploymentNetwork, ...(fixtures ? [uiFixtureBuiltin] : [])], coreRootIds: [CONSOLE_CORE_ID],
