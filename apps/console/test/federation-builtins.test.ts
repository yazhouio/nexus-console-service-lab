...
const { loadRemote, createInstance } = vi.hoisted(() => {
  ...
  return { loadRemote, createInstance: vi.fn(() => ({ loadRemote })) };
});
...
it('collects partial success without activation and reuses its Federation instance', async () => {
  loadRemote
    .mockResolvedValueOnce({ plugin, css: ['https://cdn.test/v1/a.css'] })
    .mockRejectedValueOnce(Error('404'));
  const load = prepare([pin, { ...pin, id: 'other', name: 'other_v1' }]);
  const result = await load();
  expect(result.builtins).toEqual([plugin]);
...
it('stops waiting and ignores late results', async () => {
  ...
  expect(result.failures[0].stage).toBe('timeout');
  resolve({ plugin, css: [] });
