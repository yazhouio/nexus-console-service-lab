...
export const consoleCore: PluginDefinition = {
  ...
  activate({ contributions, capabilities, actions }) {
    contributions.registerSurface({
      id: CONSOLE_ROOT_SURFACE,
      target: { kind: 'builtin', render: ConsoleLayout },
    });
    ...
    actions.register('plugin-status', async ({ context, capabilities }) => {
      ...
      const plugins = (await capabilities.invoke(
        'plugins.query@1',
        'list',
        null,
    ...
    contributions.registerRoute({
      ...consoleCoreRoutes['host-overview'],
      target: { kind: 'builtin', render: Overview },
    });
    contributions.registerRoute({
      ...consoleCoreRoutes['console-settings'],
      target: { kind: 'builtin', render: Settings },
    });
