...
import { CONSOLE_CORE_ID, CONSOLE_CORE_CAPABILITY, CORE_ROUTES_POINT, PLUGIN_DETAILS_ACTIONS_POINT, primaryNavigation } from '@nexus/console-core-api';
...
export const consoleCoreNavigation = {
  'host-overview-navigation': primaryNavigation({ id: 'host-overview-navigation', label: 'Overview', routeId: 'host-overview', order: 0 }),
  'console-settings-navigation': primaryNavigation({ id: 'console-settings-navigation', label: 'Settings', routeId: 'console-settings', group: 'system', order: 1000 }),
} as const satisfies Record<string, NavigationContribution>;

export const consoleCoreExtensions = {
  'plugin-status': { id: 'plugin-status', kind: 'action', actionId: 'plugin-status', label: 'Check plugin status', point: PLUGIN_DETAILS_ACTIONS_POINT },
