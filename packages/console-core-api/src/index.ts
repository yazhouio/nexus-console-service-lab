...
export const CORE_ROUTES_POINT = { ownerPluginId: CONSOLE_CORE_ID, id: 'routes', contractMajor: 1 } as const satisfies ExtensionPointRef;
export const PRIMARY_NAVIGATION_POINT = { ownerPluginId: CONSOLE_CORE_ID, id: 'primary-navigation', contractMajor: 1 } as const satisfies ExtensionPointRef;
export const HOME_CARDS_POINT = { ownerPluginId: CONSOLE_CORE_ID, id: 'home.cards', contractMajor: 1 } as const satisfies ExtensionPointRef;
export const SETTINGS_SECTIONS_POINT = { ownerPluginId: CONSOLE_CORE_ID, id: 'settings.sections', contractMajor: 1 } as const satisfies ExtensionPointRef;
export const PLUGIN_DETAILS_ACTIONS_POINT = { ownerPluginId: CONSOLE_CORE_ID, id: 'plugin-details.actions', contractMajor: 1 } as const satisfies ExtensionPointRef;
export interface PluginRef { readonly id: string }
export interface ListContext<Ref> { readonly selectedRefs: readonly Ref[]; readonly filtered: boolean; readonly matchedCount?: number }
export interface DetailContext<Ref> { readonly itemRef: Ref }
...
export const CONSOLE_PROFILES = [
  ...
  { id: 'console-core.navigation@1', kind: 'navigation', contextSchema: empty, allowedConstraints: { groups: ['primary','secondary','system'], order: constraints.order } },
  { id: 'console-core.home.cards@1', kind: 'surface', contextSchema: empty, allowedConstraints: constraints },
  { id: 'console-core.settings.sections@1', kind: 'surface', contextSchema: empty, allowedConstraints: { ...constraints, sizing: { modes: ['bounded'], minHeight: 100, maxHeight: 600 } } },
  { id: 'list.actions@1', kind: 'action', refParameters: ['itemRefContract'], contextSchema: { type: 'object', properties: { selectedRefs: { type: 'array', maxItems: 100, items: { $refContract: 'itemRefContract' } }, filtered: { type: 'boolean' }, matchedCount: { type: 'integer', minimum: 0 } }, required: ['selectedRefs','filtered'], additionalProperties: false }, payloadSchema: { type: 'null' }, allowedConstraints: constraints, allowedPolicyDimensions: ['selectionCount','capability','trait','predicate'] },
  { id: 'detail.actions@1', kind: 'action', refParameters: ['itemRefContract'], contextSchema: detail, payloadSchema: { type: 'null' }, allowedConstraints: constraints, allowedPolicyDimensions: ['capability','trait','predicate'] },
  { id: 'detail.tabs@1', kind: 'tab', refParameters: ['itemRefContract'], contextSchema: detail, allowedConstraints: { ...constraints, cardinality: { min: 0, max: 1 } } },
] as const satisfies readonly PointProfile[];
export function consoleRoute(value: Omit<RouteContribution, 'point'>): RouteContribution { return { ...value, point: CORE_ROUTES_POINT }; }
export function primaryNavigation(value: Omit<NavigationContribution, 'point'>): NavigationContribution { return { ...value, point: PRIMARY_NAVIGATION_POINT }; }
export function homeCard(value: Omit<SurfaceContributionDefinition, 'kind' | 'point'>): SurfaceContributionDefinition { return { ...value, kind: 'surface', point: HOME_CARDS_POINT }; }
export function settingsSection(value: Omit<SurfaceContributionDefinition, 'kind' | 'point'>): SurfaceContributionDefinition { return { ...value, kind: 'surface', point: SETTINGS_SECTIONS_POINT }; }
