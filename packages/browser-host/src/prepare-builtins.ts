...
export function mergePreparedBuiltins(
  local: readonly PluginDefinition[],
  css: Readonly<Record<string, readonly string[]>> = {},
  prepared?: PreparedBuiltins,
) {
  ...
  const builtinCss = Object.freeze(
    Object.fromEntries(
      Object.entries({ ...css, ...prepared?.builtinCss }).map(([id, urls]) => [
        id,
        Object.freeze([...urls]),
      ]),
    ),
  ...
  return {
    ...
    builtinCss,
    failures,
    filterInstallations(records: readonly InstalledPluginRecord[]) {
      const conflicts = Object.freeze(
        records
          .filter((record) => reserved.has(record.manifest.id))
          .map((record) =>
            Object.freeze({
              id: record.manifest.id,
              version: record.manifest.version,
              reason: 'Identity reserved by a selected Builtin',
            }),
          ),
      );
      return {
        installed: records.filter((record) => !reserved.has(record.manifest.id)),
        conflicts,
      };
    },
  };
