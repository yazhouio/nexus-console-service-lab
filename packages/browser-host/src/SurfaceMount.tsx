...
export function SurfaceMount({ pluginId, target, mountPointId, label, testId, routeContext, onFailure, autoMount = false }: {
  ...
  const failed = useRef(onFailure); failed.current = onFailure;
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      setPresentation(execution.phase === 'failed' ? { state: 'ERROR', stage: execution.stage } : { state: execution.phase === 'ready' ? 'MOUNTED' : 'MOUNTING' });
    };
  ...
  return <section aria-label={label}>
    ...
    {opened && presentation.state === 'ERROR' && <button onClick={() => { uiRootRef.current?.retry(); }}>Retry {label}</button>}
