...
function RoutePage({ route }: { readonly route: ResolvedRoute }) {
  ...
  if (
    ...
    (resolution.state === 'CONFLICT' && !resolution.ancestors.includes(route.contribution.id))
  ) {
    return (
      <section role="alert">
        <h1>Route unavailable</h1>
        {(resolution.state === 'CONFLICT' ? resolution.diagnostics : route.diagnostics).map(
          (d, i) => (
            <p key={i}>{d.code}</p>
          ),
        )}
    ...
    params: Object.fromEntries(
      Object.entries(bindings).filter(
        (p): p is [string, string] =>
          p[1] !== undefined &&
          Boolean(
            route.space?.segments.some((s) => s.kind === 'param' && s.name === p[0]) ||
            (p[0] === '*' && route.space?.splat),
          ),
      ),
    ),
  };
  if (target.kind === 'builtin') {
    return (
      <section data-route-id={context.routeId} data-route-params={JSON.stringify(context.params)}>
        <ManagedBuiltin
          ui={ui}
          ownerPluginId={route.ownerPluginId}
          surfaceId={route.contribution.id}
          target={target}
...
export function RoutedContent({ model }: { readonly model: RouteModel }) {
  ...
  if (!content && resolution.state === 'CONFLICT')
    return (
      <section role="alert">
        <h1>Route unavailable</h1>
