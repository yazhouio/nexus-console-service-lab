...
import {
  ...
  createUiOverlayPlugin,
  inspect,
  platformBridgeContracts,
  resolveRootPresentation,
...
type Startup =
  | { state: 'BOOTSTRAPPING' }
  | { state: 'FAILED'; failure: BootstrapFailure; preparation: unknown }
  | { state: 'READY'; services: HostServices };
...
export function BrowserHost({ distribution }: { readonly distribution: BrowserDistribution }) {
  ...
  useEffect(() => {
    ...
    setStartup({ state: 'BOOTSTRAPPING' });
    setPresentationFailure(undefined);
    const platform = createPlatformPlugin(),
      overlay = createUiOverlayPlugin();
    const bridgeContracts = [
      ...platformBridgeContracts,
      uiOverlayBridgeContract,
      ...(distribution.bridgeContracts ?? []),
    ];
    let preparation: unknown = Object.freeze({
    ...
    void Promise.resolve()
      .then(async () => {
        ...
        preparation = Object.freeze({
          failures: Object.freeze(
            (prepared?.failures ?? []).map((failure) => Object.freeze({ ...failure })),
          ),
          conflicts: Object.freeze([]),
        });
        const merged = mergePreparedBuiltins(
          [platform.plugin, overlay.plugin, ...distribution.builtins],
          distribution.builtinCss,
          prepared,
        );
        const policy = createContributionPolicy(distribution.policyBundle);
        const store = distribution.createStore(bridgeContracts);
        const { installed, conflicts } = merged.filterInstallations([
          ...store.list(),
          ...(distribution.additionalInstallations ?? []),
        ]);
        ...
        const adapter = browser.createWujiePluginAdapter({
          runtime,
          onLifecycle(event) {
        ...
        const ui = browser.createUiHost({
          onError(cause, attemptId) {
            presentationErrors.push({ attemptId, message: String(cause) });
            if (presentationErrors.length > 100) presentationErrors.shift();
          },
          onAudit: appendAudit,
          runtime,
          restrictedAdapter: adapter,
          policy,
          builtinPermissions: distribution.capabilityGrants,
          builtinCss: merged.builtinCss,
          renderBuiltin: renderBuiltinUi,
        });
        ...
        const model = createRouteModel({
          ...
          navigation: runtime.contributions.listNavigation(),
          points: runtime.contributions.listExtensionPoints(),
          rootRoutePoint: runtime.rootRoutePoint,
          navigationRootPoints: runtime.navigationRootPoints,
          policy,
        });
        const diagnostics = () => ({
          ...inspect(runtime, {
            listInstances: () => adapter.listInstances(),
            inspectUi: () => ui.core.inspect(),
            inspectActions: () => ui.actions.inspect(),
            listHostContributions: () =>
              model.listHostContributions(window.location.pathname + window.location.search),
          }),
          preparation,
        ...
        setStartup({
          state: 'READY',
          services: { runtime, store, adapter, ui, model, audit, platform, diagnostics },
        });
      })
      .catch((error) => {
        cleanup?.();
        if (!disposed)
          setStartup({ state: 'FAILED', failure: { ready: false, error }, preparation });
      });
    return () => {
      disposed = true;
      cleanup?.();
  ...
  if (startup.state === 'FAILED')
    return (
      <BreakGlass
        ready={false}
        diagnostics={{ ...inspect(startup.failure), preparation: startup.preparation }}
        recovery={distribution.recovery}
        applicationLabel={distribution.applicationLabel}
      />
    );
  const { services } = startup;
  let root;
  try {
    root = resolveRootPresentation(services.runtime);
  } catch {
    return (
      <BreakGlass
        ready
  ...
  return (
    <HostContext value={services}>
      <BrowserRouter>
        <BrowserPresentation
          services={services}
          root={root}
          onFailure={() => setPresentationFailure(services.diagnostics())}
        />
      </BrowserRouter>
    </HostContext>
  );
...
function BrowserPresentation({
  ...
  root: ReturnType<typeof resolveRootPresentation>;
  onFailure(): void;
}) {
  const location = useLocation();
  useEffect(() => {
    services.platform.notify('routes.query@1');
  }, [services, location.pathname, location.search]);
  return (
    <RouteLinkProvider
      renderLink={(props, client) => {
        let href: string;
        try {
          href = services.model.pathFor(props.routeId, props.params);
        } catch {
          return (
            <span className={props.className} aria-disabled="true">
              {props.children}
            </span>
          );
        }
        return (
          <a
            ...
            onClick={(event) => {
              ...
              void client
                .invoke('routes.navigate@1', 'navigate', {
    ...
    >
      {root.contribution.target.kind === 'builtin' ? (
        <ManagedBuiltin
          ui={services.ui}
          ownerPluginId={root.ownerPluginId}
          surfaceId={root.contribution.id}
          target={root.contribution.target}
          mountPointId="root-presentation"
          onFailure={onFailure}
          outlet={<RoutedContent model={services.model} />}
        />
      ) : (
        <SurfaceMount
