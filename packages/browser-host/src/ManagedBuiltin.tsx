...
export function renderInTree(render: unknown, container: HTMLElement, client: UiClient, fail: (error: unknown) => void): Promise<UiMounted> | undefined {
  const publish = containers.get(container);
  if (!publish) return undefined;
  return new Promise(resolve => {
    const presentation: Presentation = {
      id: crypto.randomUUID(), render, client, fail,
...
export function ManagedBuiltin({ ui, ownerPluginId, surfaceId, target, mountPointId, routeContext, onFailure, outlet }: {
  ui: UiHost; ownerPluginId: string; surfaceId: string; target: BuiltinRenderTarget; mountPointId: string;
  routeContext?: RouteContext; onFailure?: () => void; outlet?: ReactNode;
}) {
  ...
  useLayoutEffect(() => {
    ...
    return () => { active = false; stop(); containers.delete(element); current?.committed(); root.dispose(); };
