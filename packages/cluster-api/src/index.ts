...
export const RESOURCE_REF_CONTRACT = {
  ...
  schema: {
    ...
    required: ['clusterId', 'apiVersion', 'kind', 'name'],
    additionalProperties: false,
  },
  traits: { uid: { field: 'uid' }, kind: { field: 'kind' } },
  predicates: { hasIdentity: { trait: 'uid', op: 'present' } },
} as const satisfies RefContract;
export const NODE_CHILD_ROUTES_POINT = {
  ownerPluginId: 'cluster',
  id: 'node.children',
  contractMajor: 1,
} as const satisfies ExtensionPointRef;
export const NODE_NAVIGATION_POINT = {
  ownerPluginId: 'cluster',
  id: 'node.navigation',
  contractMajor: 1,
} as const satisfies ExtensionPointRef;
export const NODE_ACTIONS_POINT = {
  ownerPluginId: 'cluster',
  id: 'node.actions',
  contractMajor: 1,
} as const satisfies ExtensionPointRef;
export const NODE_TABS_POINT = {
  ownerPluginId: 'cluster',
  id: 'node.tabs',
  contractMajor: 1,
} as const satisfies ExtensionPointRef;
...
export const CLUSTER_EXTENSION_POINTS = [
  {
    id: NODE_CHILD_ROUTES_POINT.id,
    kind: 'route',
    contractMajor: 1,
    profile: 'console-core.routes@1',
  },
  {
    id: NODE_NAVIGATION_POINT.id,
    kind: 'navigation',
    contractMajor: 1,
    profile: 'console-core.navigation@1',
  },
  {
    id: NODE_ACTIONS_POINT.id,
    kind: 'action',
    contractMajor: 1,
    profile: 'detail.actions@1',
    bindings: { itemRefContract: RESOURCE_REF_CONTRACT.id },
  },
  {
...
export const CLUSTER_EXTENSION_POINT_CATALOG = [
  {
    ref: NODE_CHILD_ROUTES_POINT,
    kind: 'route',
    title: 'Node child routes',
    description: 'Extend the node detail route with a child page.',
  },
  {
    ref: NODE_NAVIGATION_POINT,
    kind: 'navigation',
    title: 'Node navigation',
    description: 'Add a node detail navigation item.',
  },
  {
    ref: NODE_ACTIONS_POINT,
    kind: 'action',
    title: 'Node actions',
    description: 'Add a typed action for a ResourceRef.',
  },
  {
    ref: NODE_TABS_POINT,
    kind: 'tab',
    title: 'Node tabs',
    description: 'Add a lazily mounted tab for a ResourceRef.',
  },
] as const satisfies readonly ClusterExtensionPointCatalogEntry[];

/** Deployment example contracts, owned by the workload plugin. */
export const DEPLOYMENT_TABS_POINT = {
  ownerPluginId: 'deployment',
  id: 'deployment.tabs',
  contractMajor: 1,
} as const satisfies ExtensionPointRef;
export const DEPLOYMENT_CARDS_POINT = {
  ownerPluginId: 'deployment',
  id: 'deployment.cards',
  contractMajor: 1,
} as const satisfies ExtensionPointRef;
export const DEPLOYMENT_ACTIONS_POINT = {
  ownerPluginId: 'deployment',
...
export const DEPLOYMENT_EXTENSION_POINTS = [
  ...
  {
    id: DEPLOYMENT_CARDS_POINT.id,
    kind: 'surface',
    contractMajor: 1,
    profile: 'deployment.cards@1',
    bindings: { itemRefContract: RESOURCE_REF_CONTRACT.id },
  },
  {
    id: DEPLOYMENT_ACTIONS_POINT.id,
    kind: 'action',
    contractMajor: 1,
    profile: 'detail.actions@1',
    bindings: { itemRefContract: RESOURCE_REF_CONTRACT.id },
