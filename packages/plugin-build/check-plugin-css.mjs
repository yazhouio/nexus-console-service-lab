...
export function checkCss(css, namespace, { allowFixed = false } = {}) {
  ...
  root.walkDecls(declaration => {
    ...
    if (declaration.prop.startsWith('--') && !privateName(declaration.prop.slice(2))) throw Error(`Unowned variable: ${declaration.prop}`);
    if (declaration.important) throw Error('Plugin CSS cannot use !important');
    if (!allowFixed && (declaration.prop === 'z-index' || declaration.prop === 'position' && declaration.value === 'fixed')) throw Error('Use the Overlay presentation API');
  });
  ...
  root.walkRules(rule => {
    ...
    selectorParser(selectors => selectors.each(selector => {
      selector.walk(node => {
        ...
        if (node.type === 'tag' && !/^\d+$/.test(node.value) || node.type === 'id' || node.type === 'universal') throw Error(`Unowned selector: ${rule.selector}`);
        if (node.type === 'pseudo' && /:(?:root|host|global)/.test(node.value)) throw Error('Plugin CSS cannot target the document or host');
      });
      let subject = false;
      selector.each(node => {
        if (node.type === 'combinator') subject = false;
        if (node.type === 'class') { if (!privateClass(node.value)) throw Error(`Unowned class: ${node.value}`); subject = true; }
        if (node.type === 'id' || node.type === 'tag' || node.type === 'universal') throw Error(`Unowned selector: ${rule.selector}`);
        if (node.type === 'attribute' && node.attribute.startsWith('data-nexus-')) throw Error('Runtime presentation markers are private');
        if (node.type === 'pseudo' && /:(?:root|host|global)/.test(node.value)) throw Error('Plugin CSS cannot target the document or host');
      });
