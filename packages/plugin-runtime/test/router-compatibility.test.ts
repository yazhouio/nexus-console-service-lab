...
it('executes an accepted specialization chain with the pinned real matcher', () => {
  const declarations = [
    route('wide', '/clusters/*'),
    route('detail', '/clusters/:id'),
    route('current', '/clusters/current'),
  ];
  for (const routes of [declarations, [...declarations].reverse()]) {
    ...
    for (const [url, id, params] of [
      ['/clusters', 'wide', { '*': '' }],
      ['/CLUSTERS/', 'wide', { '*': '' }],
      ['/clusters/current', 'current', {}],
      ['/CLUSTERS/%63urrent///?q=x#hash', 'current', {}],
      ['/clusters/demo', 'detail', { id: 'demo' }],
      ['/clusters/caf%C3%A9', 'detail', { id: 'café' }],
      ['/clusters/a%2Fb', 'detail', { id: 'a/b' }],
      ['/clusters/a%252Fb', 'detail', { id: 'a/b' }],
      ...
      expect(model.resolve(url)).toMatchObject({
        state: 'MATCHED',
        routeId: id,
        context: { params },
      });
...
it('preserves nested match chains and inherited parameters for encoded and case variants', () => {
  const model = createRouteModel({
    routes: [
      route('node', '/clusters/:cluster/nodes/:node', {
        acceptsChildren: true,
        target: { kind: 'builtin', routeLayout: true, render: () => null },
      }),
      {
        ...route('alerts', 'alerts/:alert', { parentRouteId: 'node' }),
  ...
  const matches = matchRoutes(
    model.toRouteObjects(),
    '/CLUSTERS/demo/nodes/n%2F1/ALERTS/a%20b/?severity=high',
  );
...
it('generated owner, parameter-renaming and registration-order properties stay compatible', () => {
  for (let i = 0; i < 40; i++) {
    ...
    const conflictingOwner = createRouteModel({
      routes: declarations.map((r) =>
        r.contribution.id === 'narrow' ? { ...r, ownerPluginId: 'other' } : r,
      ),
      navigation: [],
    });
    expect(conflictingOwner.routes.map((r) => r.state)).toEqual(['QUARANTINED', 'QUARANTINED']);
    const renamed = createRouteModel({
      routes: [wide, route('renamed', `${prefix}/:other_${i}`)],
      navigation: [],
    });
    expect(renamed.routes.map((r) => r.state)).toEqual(['QUARANTINED', 'QUARANTINED']);
    const crossing = createRouteModel({
      routes: [route('a', `${prefix}/:a/end`), route('b', `${prefix}/start/:b`)],
      navigation: [],
    });
...
it.each([
  ...
  ['//x/value', 'NOT_FOUND'],
  ['/x//value', 'NOT_FOUND'],
  ['/x/value/extra', 'NOT_FOUND'],
  ['/safe', 'MATCHED'],
])('keeps quarantined URL membership consistent for %s', (url, state) => {
  const model = createRouteModel({
    routes: [route('a', '/x/:id'), route('b', '/x/:other'), route('safe', '/safe')],
    navigation: [],
  });
...
it('diagnostic placeholders cannot overwrite an available Layout parameter', () => {
  ...
  const matches = matchRoutes(
    model.toRouteObjects(() => null, true),
    '/p/parent-value/child-value',
  );
