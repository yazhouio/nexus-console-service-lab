...
it('compiles a Point binding inside its owner activation and freezes the resolved Context schema', async () => {
    ...
    builtins: [
      {
        ...
        activate({ contributions }) {
          contributions.registerExtensionPoint({
            ...
            kind: 'action',
            contractMajor: 1,
            profile: 'list.actions@1',
            bindings: { itemRefContract: ref.id },
          });
        },
      },
    ],
  ...
  expect(point).toMatchObject({
    profile: 'list.actions@1',
    bindings: { itemRefContract: 'example.plugin-ref@1' },
    contextSchema: { properties: { selectedRefs: { items: ref.schema, maxItems: 100 } } },
  });
...
it('rejects conflicting identities, schema overrides, missing bindings, and widening within an activation transaction', async () => {
  const builtins = [
    {
      ...
      requires: [],
      provides: [],
      activate({ contributions }: import('../src/index').PluginContext) {
  ...
  const runtime = await bootstrapPluginRuntime({
    ...
    profiles: [
      {
        ...profile,
        allowedConstraints: { groups: ['primary', 'secondary'], order: { min: 0, max: 10 } },
      },
    ],
    refContracts: [ref],
  });
  ...
  await expect(
    bootstrapPluginRuntime({
      ...
      builtins: [],
      profiles: [profile],
      refContracts: [ref, { ...ref, schema: { type: 'string' } }],
    }),
  ).rejects.toThrow('CONTRACT_ID_CONFLICT');
});
...
it('accepts only constraints narrowed from the Profile and retains their frozen order', async () => {
  ...
  expect(runtime.contributions.listExtensionPoints()[0].contribution.constraints).toEqual({
    groups: ['primary'],
    order: { min: 0, max: 2 },
    cardinality: { min: 0, max: 1 },
  });
...
it('checks every execution input against the compiled binding and rejects more than 100 refs', async () => {
    ...
    profiles: [
      {
        ...
        contextSchema: {
          ...
          properties: {
            ...profile.contextSchema.properties,
            selectedRefs: { type: 'array', items: { $refContract: 'itemRefContract' } },
          },
        },
      },
    ],
  ...
  const slot = ui.mountSlot(
    root.attemptId,
    {},
    { id: 'cards', contextKey: 'one', context: { selectedRefs: [{ id: 'a' }], filtered: true } },
  );
  expect(ui.snapshot(root.attemptId).occurrences[0].input.acceptance).toBe('accepted');
  ui.updateSlot(root.attemptId, slot, {
    id: 'cards',
    contextKey: 'invalid',
    context: { selectedRefs: [{ wrong: 'a' }], filtered: false },
  });
  expect(ui.snapshot(root.attemptId).occurrences[0].input.acceptance).toBe('rejected');
  ui.updateSlot(root.attemptId, slot, {
    id: 'cards',
    contextKey: 'too-many',
    context: { selectedRefs: Array.from({ length: 101 }, () => ({ id: 'a' })), filtered: false },
  });
  expect(ui.snapshot(root.attemptId).occurrences[0].input).toMatchObject({
    acceptance: 'rejected',
    effective: null,
  });
  await ui.dispose();
...
it('treats the Profile and Ref bindings as part of a Point Major across installed versions', async () => {
  ...
  const record = {
    manifest,
    config: { id: 'page', version: '1.0.0', enabled: true, grantedPermissions: [] },
  };
  const store = createInstallationStore({
    records: [record as never],
    supportedHostApis: ['example.host@1'],
    bridgeContracts: [],
    isEntryAllowed: () => true,
  });
  expect(() =>
    store.install({
      ...
      manifest: {
        ...
        version: '2.0.0',
        extensionPoints: [
          { ...manifest.extensionPoints[0], bindings: { itemRefContract: 'example.other-ref@1' } },
        ],
      },
      config: { ...record.config, version: '2.0.0' },
    }),
  ).toThrow('CONTEXT_MAJOR_FROZEN');
...
it('does not use compatibility assertions to choose another Point and bounds every JSON input', async () => {
        ...
        activate({ contributions }) {
          for (const [id, contract] of [
            ...
            contributions.registerExtensionPoint({
              ...
              contractMajor: 1,
              profile: profile.id,
              bindings: { itemRefContract: contract },
            });
        },
      },
      {
        ...
        activate({ contributions, actions }) {
          ...
          contributions.registerExtension({
            ...
            label: 'Check',
            actionId: 'check',
            point: { ownerPluginId: 'page', id: 'requested', contractMajor: 1 },
            expectedRefContract: other.id,
          });
        },
      },
    ],
  ...
  const point = { ownerPluginId: 'page', id: 'requested', contractMajor: 1 },
    context = { selectedRefs: [{ id: 'a' }], filtered: false };
  ...
  expect(() =>
    actions.invoke('page', point, { ownerPluginId: 'feature', id: 'check' }, context),
  ).toThrow('REF_CONTRACT_ASSERTION_FAILED');
  expect(() =>
    actions.query('page', point, {
      ...context,
      selectedRefs: Array.from({ length: 101 }, () => ({ id: 'a' })),
    }),
  ).toThrow('CONTEXT_INVALID');
  expect(() =>
    actions.query('page', point, { ...context, selectedRefs: [{ id: 'x'.repeat(70_000) }] }),
...
it('rejects unsupported presentation/constraint dimensions rather than accepting inert restrictions', async () => {
  ...
  expect(() =>
    compile({
      ...
      contractMajor: 1,
      contextSchema: {},
      constraints: { presentations: ['menu'] },
    }),
  ).toThrow('CONSTRAINT_NOT_SUPPORTED');
  expect(() =>
    compile({
      id: 'routes',
      kind: 'route',
      contractMajor: 1,
      contextSchema: {},
      constraints: { cardinality: { min: 0, max: 1 } },
    }),
  ).toThrow('CONSTRAINT_NOT_SUPPORTED');
  expect(() =>
    compile({
      ...
      contractMajor: 1,
      contextSchema: {},
      constraints: { presentations: ['drawer'] },
    }),
  ).toThrow('PRESENTATION_NOT_SUPPORTED');
  expect(
    compile({
      id: 'cards',
      kind: 'surface',
      contractMajor: 1,
      contextSchema: {},
      constraints: { presentations: ['inline'] },
...
it('enforces Action cardinality against its single invocation before connecting an owner', async () => {
    ...
    builtins: [
      {
        ...
        activate({ contributions, actions }) {
          contributions.registerExtensionPoint({
            ...
            contractMajor: 1,
            contextSchema: {},
            constraints: { cardinality: { min: 0, max: 0 } },
          });
          actions.register('check', () => null);
          contributions.registerExtension({
            id: 'check',
            ...
            actionId: 'check',
            label: 'Check',
            point: { ownerPluginId: 'page', id: 'none', contractMajor: 1 },
          });
        },
      },
    ],
  ...
  expect(() => actions.invoke('page', point, { ownerPluginId: 'page', id: 'check' }, {})).toThrow(
