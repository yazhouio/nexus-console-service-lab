...
import {
  ...
  type BridgeSubscriptionActionContract,
  type JsonValue,
} from '../src';
import {
...
async function fixture(
  overrides: Partial<BridgeSubscriptionActionContract> = {},
    ...
    builtins: [
      {
        ...
        version: '1',
        requires: [],
        provides: ['host.cluster@1'],
        activate(context) {
          context.capabilities.register('host.cluster@1', {});
        },
      },
    ],
    coreRootIds: ['cluster'],
    bridgeContracts: [
      {
        ...
        actions: {
          watch: {
            ...
            snapshotSchema: stringSchema,
            eventSchema: stringSchema,
            open(_capability, _payload, _context, send) {
              emit = send;
              send('changed-during-open');
  ...
  const session = createPluginBridgeSession({
    ...
    identity: {
      ...
      pluginVersion: '1',
      surfaceId: 'overview',
      surfaceInstanceId: crypto.randomUUID(),
      mountPointId: 'route:overview',
      protocolVersion: 1,
      requires: ['host.cluster@1'],
      grantedPermissions: ['cluster.read'],
    },
  });
  const received: (
    | BridgeResponse
    | { type: 'event'; subscriptionId: string; payload: JsonValue }
  )[] = [];
  ...
  const send = async (value: unknown = request): Promise<BridgeResponse> => {
    ...
    await vi.waitFor(() =>
      expect(received.slice(before).some((message) => message.type === 'response')).toBe(true),
    );
...
it('returns a snapshot before buffered changes and stops delivery before unsubscribe succeeds', async () => {
  ...
  expect(response).toMatchObject({
    ok: true,
    result: { snapshot: 'initial', subscriptionId: expect.any(String) },
  });
  const subscriptionId = (
    response.ok ? (response.result as { subscriptionId: string }) : { subscriptionId: '' }
  ).subscriptionId;
  expect(received).toEqual([
    response,
    { type: 'event', subscriptionId, payload: 'changed-during-open' },
  ]);
  expect(session.subscriptionCount).toBe(1);
  expect(await send({ type: 'unsubscribe', requestId: 'stop', subscriptionId })).toMatchObject({
    ok: true,
    result: null,
  });
  emit('too late');
  expect(
    await send({ type: 'unsubscribe', requestId: 'stop-again', subscriptionId }),
  ...
  expect(
    session.audit.map((entry) => [entry.type, entry.subscriptionId, entry.resultCode]),
  ).toEqual([
    ['subscription', subscriptionId, 'OK'],
...
it('closes only the subscription whose pre-snapshot event buffer overflows', async () => {
  ...
  const { send, received, session } = await fixture(
    {
      open(_capability, _payload, _context, emit) {
        emit('one');
        emit('two');
        return { snapshot: 'initial', dispose };
      },
...
it.each([
  ...
  'rejects invalid, oversized or excess events and audits the local failure',
  async (event, limits, code) => {
...
it.each(['snapshot', 'reject', 'timeout'] as const)(
  'never exposes an ID when open fails at %s, including late cleanup',
  async (failure) => {
    const dispose = vi.fn();
    ...
    expect(await send()).toMatchObject({
      ok: false,
      error: {
        code: { snapshot: 'INVALID_RESULT', reject: 'ACTION_FAILED', timeout: 'TIMEOUT' }[failure],
      },
    });
    complete?.();
    emit('late');
...
it('limits subscription capacity and still cleans every provider if one disposer fails', async () => {
  ...
  const { send, session } = await fixture(
    {
      open(_capability, _payload, context) {
  ...
  expect(await send({ ...request, requestId: 'third' })).toMatchObject({
    ok: false,
    error: { code: 'SUBSCRIPTION_LIMITED' },
  });
  session.dispose();
  session.dispose();
...
it('unknown unsubscribe never affects another session and buffered event schemas are applied once', async () => {
  ...
  const subscriptionId = opened.ok
    ? (opened.result as { subscriptionId: string }).subscriptionId
    : '';
  expect(
    await second.send({ type: 'unsubscribe', requestId: 'other-session', subscriptionId }),
  ).toMatchObject({ ok: true, result: null });
...
it('allows unsubscribe to release resources even when the action request rate is exhausted', async () => {
  ...
  const response = await send();
  const subscriptionId = response.ok
    ? (response.result as { subscriptionId: string }).subscriptionId
    : '';
  expect(await send({ type: 'unsubscribe', requestId: 'stop', subscriptionId })).toMatchObject({
    ok: true,
    result: null,
...
it('keeps the Host cleanup barrier pending for a late subscription and its asynchronous disposer', async () => {
  ...
  session.dispose();
  expect(await requestResult).toMatchObject({
    ok: false,
    error: { code: 'BRIDGE_SESSION_INACTIVE' },
  });
  let settled = false;
  const barrier = session.settled().then(() => {
    settled = true;
  });
  finishOpen();
