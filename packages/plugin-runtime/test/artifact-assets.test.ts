...
class Link extends EventTarget {
  ...
  dataset = {};
  removed = false;
  constructor(readonly links: Link[]) {
    super();
  }
  remove() {
    this.removed = true;
    const index = this.links.indexOf(this);
    if (index >= 0) this.links.splice(index, 1);
  }
}
function root(shadow = false) {
  const links: Link[] = [];
  const appendChild = (node: Link) => {
    links.push(node);
  };
  ...
  return {
    links,
    root: (shadow
      ? { nodeType: 11, ownerDocument: document, appendChild }
      : document) as unknown as Document | ShadowRoot,
  };
}
...
describe('Builtin artifact CSS ownership', () => {
  it('shares pending loads in artifact order and releases only the last reference', async () => {
    ...
    load(document.links[1]);
    load(document.links[0]);
    const first = await a,
      second = await b;
    first.release();
    first.release();
    expect(document.links.map((link) => link.href)).toEqual([url()]);
    second.release();
  ...
  it('keys by actual root and complete normalized URL, preserving queries and versions', async () => {
    const assets = createArtifactAssets(),
      ...
      shadow = root(true);
    const held = [
      assets.acquire([url(), url() + '?v=2', url('v2/a')], document.root, signal().signal),
      assets.acquire([url()], shadow.root, signal().signal),
      assets.acquire(['https://cdn.example/plugin/x/../a.css'], document.root, signal().signal),
    ];
    ...
    expect(document.links).toHaveLength(0);
    expect(shadow.links).toHaveLength(0);
  });
  ...
  it('cancels one pending waiter without cancelling another and detaches abort after acquisition', async () => {
    ...
    const held = await b;
    abortB.abort();
    expect(document.links).toHaveLength(1); // DOM may still be asynchronously unmounting.
    held.release();
    expect(document.links).toHaveLength(0);
  });
  it('removes a last pending reference and ignores late events from that generation', async () => {
    vi.useFakeTimers();
    const assets = createArtifactAssets(100),
      document = root(),
      abort = signal();
  ...
  it('closes admission before dispose, settles pending waiters and removes timers', async () => {
    ...
    assets.close();
    await expect(assets.acquire([url()], document.root, signal().signal)).rejects.toThrow(
      'UI_RUNTIME_CLOSED',
    );
    expect(document.links).toHaveLength(1);
    assets.dispose();
  ...
  it('rejects relative URLs and already aborted attempts before inserting any nodes', async () => {
    ...
    aborted.abort();
    await expect(
      assets.acquire(['./styles.css'], document.root, signal().signal),
    ).rejects.toThrow();
    await expect(assets.acquire([url()], document.root, aborted.signal)).rejects.toThrow(
      'STALE_EXECUTION',
    );
    expect(document.links).toHaveLength(0);
  });
