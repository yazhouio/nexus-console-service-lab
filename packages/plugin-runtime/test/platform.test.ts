...
it('uses the same versioned schemas and explicit permissions for Builtin and Restricted Route ID navigation', async () => {
  ...
  const runtime = await bootstrapPluginRuntime({
    builtins: [
      platform.plugin,
      {
        id: 'core',
        version: '1.0.0',
        requires: ['routes.query@1', 'routes.navigate@1'],
        provides: [],
        activate({ contributions }) {
          ...
          contributions.registerRoute({
            id: 'item',
            point: { ownerPluginId: 'core', id: 'routes', contractMajor: 1 },
            path: '/items/:id',
    ...
    coreRootIds: ['core'],
    bridgeContracts: platformBridgeContracts,
  });
  ...
  const model = createRouteModel({
    ...
    rootRoutePoint: { ownerPluginId: 'core', id: 'routes', contractMajor: 1 },
  });
  ...
  platform.bind({
    ...
    audit: () => [],
    recordAudit: (value) => audit.push(value),
  });
  for (const grantedPermissions of [[], ['routes.query', 'routes.navigate']]) {
    ...
    const response = await session.dispatch({
      ...
      action: 'navigate',
      payload: { routeId: 'item', params: { id: 'a b' } },
    });
    ...
    if (grantedPermissions.length) {
      ...
      const query = await session.dispatch({
        type: 'request',
        requestId: '3',
      ...
      expect(query).toMatchObject({
        ok: true,
        result: [{ routeId: 'item', ownerPluginId: 'core', ancestry: ['item'] }],
      });
    }
    session.dispose();
    ports.port2.close();
...
it('cannot bypass platform grants with a forged Builtin invocation context and keeps Builtin/Core management read-only', async () => {
  ...
  platform.bind({
    runtime,
    model: createRouteModel({ routes: [], navigation: [] }),
    store,
    location: () => '/',
    navigate() {},
    audit: () => [],
  ...
  const capability = runtime.capabilities.require('plugins.manage@1') as {
    invoke(action: string, payload: unknown, context: unknown): unknown;
  };
  expect(() =>
    capability.invoke(
      'uninstall',
      { id: 'core' },
      { pluginId: 'core', signal: new AbortController().signal },
    ),
  ).toThrow('PLATFORM_INVOCATION_UNBOUND');
  const ports = new MessageChannel();
  const session = createPluginBridgeSession({
    runtime,
    port: ports.port1,
    identity: {
      ...
      pluginVersion: '1.0.0',
      surfaceId: 'page',
      surfaceInstanceId: 'attempt',
      mountPointId: 'page',
      protocolVersion: 1,
      requires: ['plugins.manage@1'],
      grantedPermissions: ['plugins.manage'],
    },
  });
  let sequence = 0;
  for (const id of ['core', 'builtin-feature'])
    for (const [action, payload] of [
      ['uninstall', { id }],
      ['setEnabled', { id, enabled: false }],
      ['selectVersion', { id, version: '2.0.0' }],
      ['install', { id, version: '2.0.0' }],
    ] as const)
      expect(
        await session.dispatch({
          type: 'request',
          requestId: String(++sequence),
          capability: 'plugins.manage@1',
          action,
          payload,
        }),
