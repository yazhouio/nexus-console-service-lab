...
it('ignores transport reordering, connects the full snapshot progress point, and preserves the retry baseline', async () => {
  ...
  const state = (progress: number): UiObservation => ({
    streamId: 'attempt-2',
    progress,
    initialContext: { contextKey: 'A', revision: 2, value: 2 },
    context: { contextKey: 'A', revision: 3, value: 3 },
    overlayInput: null,
    occurrences: [],
  });
  const refresh = client.refresh();
  emit({ type: 'ui:snapshot', snapshot: state(8) });
  emit({ type: 'ui:response', requestId: sent[0].requestId, ok: true, result: state(6) });
  await refresh;
  ...
  const stop = client.watchContext((value) => values.push(value?.value));
...
it('preserves the highest Host Overlay progress while events race the subscription response', async () => {
  ...
  emit({
    ...
    ok: true,
    result: { subscriptionId: 'one', snapshot: snapshot(8) },
  });
...
it('calls versioned capabilities without a UI snapshot and scopes subscriptions to the client session', async () => {
  ...
  emit({
    ...
    ok: true,
    result: { subscriptionId: 'plugins', snapshot: ['old'] },
  });
  const stop = await opening;
