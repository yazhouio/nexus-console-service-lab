...
function successfulHandshake(ports: MessagePort[] = []): BridgeHandshakeCoordinator {
  return {
...
describe('WujiePluginAdapter', () => {
  it('injects a JSON route snapshot independently from static Surface configuration', async () => {
    ...
    expect(props).toMatchObject({
      routeContext: { params: { node: 'n1' }, search: '?severity=high' },
      surface: { initialParameters: { view: 'route' } },
    });
  ...
  it('cancels an in-flight start, disposes first, and destroys its late result once', async () => {
    ...
    const adapter = createWujiePluginAdapter({
      ...
      hostWindow: hostWindow(),
      createSurfaceInstanceId: () => 'cancelled',
    });
    const abort = new AbortController();
    const mounting = adapter.mount({
      pluginId: 'kubeeye',
      surfaceId: 'overview',
      mountPointId: 'route:cancelled',
      container,
      signal: abort.signal,
    });
    const rejection = expect(mounting).rejects.toMatchObject({
      issue: { code: 'SURFACE_MOUNT_CANCELLED' },
    });
  ...
  it('contains a render error after mount and cleans iframe listeners', async () => {
    ...
    const adapter = createWujiePluginAdapter({
      ...
      handshake: successfulHandshake([port]),
      hostWindow: hostWindow(),
      createSurfaceInstanceId: () => 'render-failure',
      onSurfaceError,
    });
    const mounted = await adapter.mount({
      pluginId: 'kubeeye',
      surfaceId: 'overview',
      mountPointId: 'route:render',
      container,
    });
    iframe.dispatchEvent(new Event('error'));
    await vi.waitFor(() => expect(destroy).toHaveBeenCalledOnce());
    expect(port.close).toHaveBeenCalledOnce();
    expect(adapter.getInstance('render-failure')?.state).toMatchObject({
      state: 'FAILED',
      stage: 'render',
    });
            ...
            : {
                begin: () => ({
                  result: Promise.reject(
                    new BridgeHandshakeError('BRIDGE_NONCE_INVALID', 'invalid nonce'),
                  ),
                  cancel() {},
                }),
              },
        hostWindow: hostWindow(),
        createSurfaceInstanceId: () => `${stage}-failure`,
      });
      await expect(
        adapter.mount({
          pluginId: 'kubeeye',
          surfaceId: 'overview',
          mountPointId: `route:${stage}`,
          container,
        }),
        ...
        issue: {
          ...
          pluginId: 'kubeeye',
          surfaceId: 'overview',
          surfaceInstanceId: `${stage}-failure`,
          mountPointId: `route:${stage}`,
        },
      });
  ...
  it('does not reuse an instance id after unmount', async () => {
    const adapter = createWujiePluginAdapter({
      ...
      handshake: successfulHandshake(),
      hostWindow: hostWindow(),
      createSurfaceInstanceId: () => 'fixed',
    });
    const input = {
      pluginId: 'kubeeye',
      surfaceId: 'overview',
      mountPointId: 'route:id',
      container,
    };
  ...
  it('rejects Host objects in metadata and clones allowed JSON before Wujie injection', async () => {
    ...
    const adapter = createWujiePluginAdapter({
      runtime: await runtime(),
      driver: { startApp, async destroyApp() {} },
      handshake: successfulHandshake(),
      hostWindow: hostWindow(),
    });
    const input = {
      pluginId: 'kubeeye',
      surfaceId: 'overview',
      mountPointId: 'route:props',
      container,
    };
    await expect(
      adapter.mount({ ...input, initialParameters: { client: () => 'host' } as never }),
    ...
    expect(startApp.mock.calls[0]?.[0].props?.surface.initialParameters).toEqual({
...
it('projects session failure only on the owning instance and preserves earlier snapshots', async () => {
  ...
  try {
    ...
    await adapter.mount({
      pluginId: 'kubeeye',
      surfaceId: 'overview',
      mountPointId: 'extension:cards/overview',
      container,
    });
    ...
    await vi.waitFor(() =>
      expect(adapter.getInstance('instance-1')?.state).toMatchObject({
        state: 'FAILED',
        stage: 'bridge',
      }),
    );
    ...
    expect(plugin?.surfaces?.[0].instances).toMatchObject([
      {
        ...
        state: 'FAILED',
        bridgeSession: { state: 'DISPOSED', subscriptionCount: 0 },
      },
      {
        surfaceInstanceId: 'instance-2',
        state: 'MOUNTED',
        bridgeSession: { state: 'ACTIVE', subscriptionCount: 0 },
      },
    ]);
    expect(
      before.plugins
        .find((plugin) => plugin.id === 'kubeeye')
        ?.surfaces?.[0].instances.every((instance) => instance.state === 'MOUNTED'),
    ).toBe(true);
    const response = new Promise((resolve) => {
      second.port2.onmessage = (event) => resolve(event.data);
    });
    second.port2.postMessage({
      type: 'request',
      requestId: 'bad-1',
...
it('loads an Action-only owner without a Surface and closes its per-invocation bridge', async () => {
  ...
  const adapter = createWujiePluginAdapter({
    runtime: await runtime(true),
    hostWindow: hostWindow(),
    handshake: successfulHandshake([ports.port1]),
    driver: {
      async startApp(options) {
        props = options.props;
  ...
  ports.port2.addEventListener('message', (event) => {
    if (event.data.type === 'action:execute')
      ports.port2.postMessage({
        type: 'ui:request',
        requestId: 2,
        action: 'action.complete',
        payload: { ok: true, result: { checked: true } },
      });
  });
  ports.port2.start();
  const session = await adapter.connectAction(
    {
      ownerPluginId: 'kubeeye',
      actionId: 'check',
