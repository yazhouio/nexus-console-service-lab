      ...
      routes: [
        {
          ...
          path: '/external',
          surfaceId: 'page',
          initialParameters: { token: 'private initial data' },
        },
      ],
      extensions: [
        {
          id: 'unused',
          kind: 'surface',
          point: { ownerPluginId: 'host', id: 'missing', contractMajor: 1 },
          surfaceId: 'page',
        },
      ],
    },
...
it('projects ownership and inactive surfaces without exposing providers, render functions or parameters', async () => {
    ...
    builtins: [
      {
        ...
        activate(context) {
          context.capabilities.register('host.cluster@1', { token: 'private capability' });
        },
      },
    ],
    coreRootIds: ['cluster'],
  ...
  expect(snapshot.plugins).toContainEqual(
    expect.objectContaining({
      ...
      requestedPermissions: ['cluster.read'],
      grantedPermissions: [],
      surfaces: [{ id: 'page', instances: [] }],
    }),
  );
  expect(() => Object.assign(runtime.resolution.dependencies[0], { provider: 'forged' })).toThrow();
  expect(snapshot.dependencies).toEqual([
    { consumer: 'external', capability: 'host.cluster@1', provider: 'cluster' },
  ]);
  expect(snapshot.capabilities).toEqual([{ id: 'host.cluster@1', providerPluginId: 'cluster' }]);
  expect(snapshot.contributions.extensions).toContainEqual(
    expect.objectContaining({
      id: 'unused',
      point: { ownerPluginId: 'host', id: 'missing', contractMajor: 1 },
...
it('sanitizes bootstrap and plugin failures while retaining dependency skip reasons', async () => {
  ...
  const failed = await bootstrapPluginRuntime({ builtins: [broken], coreRootIds: ['broken'] }).then(
    () => {
      throw Error('Expected bootstrap failure');
    },
    (error) => inspect({ ready: false, error }),
  );
  expect(failed).toMatchObject({
    ready: false,
    bootstrapError: { code: 'PLUGIN_ACTIVATION_FAILED', message: 'Plugin activation failed.' },
  });
  const runtime = await bootstrapPluginRuntime({
    builtins: [
      broken,
  ...
  expect(snapshot.plugins).toContainEqual(
    expect.objectContaining({
      id: 'broken',
      state: 'FAILED',
      error: { code: 'PLUGIN_ACTIVATION_FAILED', message: 'Plugin activation failed.' },
    }),
  );
  expect(snapshot.plugins).toContainEqual(
    expect.objectContaining({
      id: 'skipped',
      state: 'SKIPPED',
      error: expect.objectContaining({ code: 'MISSING_CAPABILITY' }),
    }),
  );
  expect(JSON.stringify([failed, snapshot])).not.toMatch(
    /secret-token|Bearer|stack|private provider/,
  );
});
...
it('keeps safe plugin, stage and dependency attribution when Core bootstrap fails', async () => {
  const snapshot = await bootstrapPluginRuntime({
    builtins: [
      { id: 'core-shell', version: '1', requires: ['missing.api@1'], provides: [], activate() {} },
    ],
  ...
  expect(snapshot.bootstrapError).toMatchObject({
    ...
    capability: 'missing.api@1',
    path: ['core-shell', 'missing.api@1'],
  });
    ...
    builtins: [
      {
        ...
        provides: [],
        activate() {
          throw Error('Bearer secret-token');
        },
      },
    ],
    coreRootIds: ['core-shell'],
  ...
  expect(activation.bootstrapError).toMatchObject({
