...
const input = (contextKey = 'A', value: number | string = 1): SlotInput => ({
  id: 'details',
  contextKey,
  context: { value },
});
function fixture() {
  ...
  for (const owner of ['a', 'b', 'c']) {
    ...
    activation.context.registerExtensionPoint({
      id: 'details',
      kind: 'surface',
      contractMajor: 1,
    ...
    if (owner !== 'a')
      activation.context.registerExtension({
        ...
        kind: 'surface',
        surfaceId: 'main',
        point: { ownerPluginId: owner === 'b' ? 'a' : 'b', id: 'details', contractMajor: 1 },
      });
    activation.commit();
  }
  ...
  const execution = (occurrenceId: string) =>
    core.snapshot(root.attemptId).occurrences.find((o) => o.occurrenceId === occurrenceId)!
      .contributions[0].execution!;
...
describe('Host UI composition contracts', () => {
  it('retains same-key executions, separates input rejection, and never resurrects a visited key', async () => {
    ...
    expect(f.core.snapshot(f.root.attemptId).occurrences[0].input).toMatchObject({
      acceptance: 'rejected',
      effective: { value: { value: 2 } },
    });
    ...
    expect(
      f.core.snapshot(f.root.attemptId).occurrences[0].contributions[0].execution,
    ).toBeUndefined();
    ...
    expect(f.core.inspect().scopes).toHaveLength(0);
  });
  it('hidden is not keep-alive; separate occurrences never share execution', async () => {
    const f = fixture(),
      o1 = f.slot(),
      o2 = f.slot();
    const a1 = f.execution(o1).attemptId,
      a2 = f.execution(o2).attemptId;
    expect(a1).not.toBe(a2);
    const revision = f.core.snapshot(f.root.attemptId).occurrences[0].input.effective!.revision;
    f.core.updateSlot(f.root.attemptId, o1, {
      ...input(),
      hidden: true,
      sizing: { mode: 'bounded' },
    });
    expect(f.execution(o1).attemptId).toBe(a1);
    expect(f.core.snapshot(f.root.attemptId).occurrences[0].input.effective!.revision).toBe(
      revision,
    );
    f.core.unmountSlot(f.root.attemptId, o1);
    expect(() => f.core.snapshot(a1)).toThrow();
    const o3 = f.slot();
    expect(f.execution(o3).attemptId).not.toBe(a1);
    expect(f.execution(o2).attemptId).toBe(a2);
    await f.core.dispose();
  });
  it('captures retry Context at acceptance and then delivers later same-key updates', async () => {
    const f = fixture(),
      o = f.slot();
    await vi.waitFor(() => expect(f.execution(o).phase).toBe('ready'));
    const failed = f.execution(o).attemptId;
    f.core.failAttempt(failed);
    f.core.updateSlot(f.root.attemptId, o, input('A', 2));
    let release!: () => void;
    f.driver.mount = async () => {
    ...
    expect(f.core.snapshot(retried)).toMatchObject({
      initialContext: { value: { value: 2 } },
      context: { value: { value: 3 } },
    });
  ...
  it('keeps an owner-scope Overlay across retry but cascades child scopes on parent Attempt failure', async () => {
    ...
    f.core.failAttempt(f.root.attemptId);
    f.root.retry();
    expect(f.core.overlaySnapshot(f.root.attemptId).overlays.map((x) => x.handle)).toEqual([
      rootHandle,
    ]);
    expect(() => f.core.overlaySnapshot(b2, bHandle)).toThrow('STALE_EXECUTION');
    await f.core.dispose();
    expect(f.core.inspect().overlays).toBe(0);
  });
  it('commits one Overlay terminal outcome and retains it until owner Scope ends', async () => {
    const f = fixture(),
      handle = f.core.openOverlay(f.root.attemptId, 'main', { id: 1 });
    const overlayAttempt = f.core.overlaySnapshot(f.root.attemptId, handle).overlays[0].execution!
      .attemptId;
    f.core.completeOverlay(overlayAttempt, { saved: true });
    f.core.cancelOverlay(f.root.attemptId, handle);
    expect(f.core.overlaySnapshot(f.root.attemptId, handle).overlays[0]).toMatchObject({
      outcome: { state: 'completed', result: { saved: true } },
    });
    expect(f.core.overlaySnapshot(f.root.attemptId, handle).overlays[0].execution).toBeUndefined();
    await f.core.dispose();
    expect(() => f.core.overlaySnapshot(f.root.attemptId, handle)).toThrow();
  });
  it('invalidates before cleanup and disposes resources that finish mounting late', async () => {
    const f = fixture();
    await vi.waitFor(() => expect(f.root.execution.phase).toBe('ready'));
  ...
  it('assigns Host progress independently of Context revision and returns full recovery snapshots', async () => {
    const f = fixture(),
      o = f.slot();
    const before = f.core.snapshot(f.root.attemptId);
    ...
    expect(after.occurrences[0].input.effective?.revision).toBe(
      before.occurrences[0].input.effective?.revision,
    );
    expect(after.occurrences[0].visibility).toBe('hidden');
    await f.core.dispose();
  });
  it('isolates unauthorized, mismatched and missing relations while filtering never grants execution', async () => {
    ...
    a.context.registerExtension({
      ...
      kind: 'surface',
      surfaceId: 'main',
      point: { ownerPluginId: 'a', id: 'details', contractMajor: 1 },
    });
    a.context.registerExtension({
      id: 'new-major',
      kind: 'surface',
      surfaceId: 'main',
      point: { ownerPluginId: 'a', id: 'details', contractMajor: 2 },
    });
    a.context.registerExtension({
      id: 'missing',
      kind: 'surface',
    ...
    const core = createUiRuntime({
      ...
      policy(request) {
        ...
        return (
          request.kind === 'surface' && request.contributorId === 'b' && request.contractMajor === 1
        );
      },
    });
    ...
    expect(
      core.snapshot(root.attemptId).occurrences[0].contributions.map((c) => c.ref.ownerPluginId),
    ).toEqual(['b']);
    core.updateSlot(root.attemptId, o, {
      ...input(),
      selected: [{ ownerPluginId: 'other', id: 'ungranted' }],
    });
    ...
    const allAllowed = createUiRuntime({
      registry: f.registry.registry,
      driver: f.driver,
      policy: () => true,
    });
    const r = allAllowed.attachOwner('a', {});
    allAllowed.mountSlot(r.attemptId, {}, input());
    expect(allAllowed.snapshot(r.attemptId).occurrences[0].contributions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ref: { ownerPluginId: 'other', id: 'new-major' },
          reason: 'CONTRACT_MISMATCH',
        }),
        expect.objectContaining({
          ref: { ownerPluginId: 'other', id: 'missing' },
    ...
    expect(
      allAllowed
        .snapshot(r.attemptId)
        .occurrences[0].contributions.filter((c) => c.availability === 'unavailable')
        .every((c) => !c.execution),
    ).toBe(true);
