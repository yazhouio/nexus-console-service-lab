...
export interface PointConstraints {
  ...
  readonly sizing?: { readonly modes: readonly ('content-sized' | 'bounded')[]; readonly minWidth?: number; readonly maxWidth?: number; readonly minHeight?: number; readonly maxHeight?: number };
...
export type ActionCondition =
  | { readonly source: 'selectionCount'; readonly op: ComparisonOperator; readonly value?: import('../contribution').JsonValue }
  | { readonly source: 'capability'; readonly id: ContractId; readonly available: boolean }
  | { readonly source: 'trait'; readonly name: string; readonly op: ComparisonOperator; readonly value?: import('../contribution').JsonValue; readonly match: 'all' | 'any' }
  | { readonly source: 'predicate'; readonly name: string; readonly value: boolean; readonly match: 'all' | 'any' };
export interface RefContract {
  readonly traits?: Readonly<Record<string, { readonly field: string }>>;
  readonly predicates?: Readonly<Record<string, { readonly trait: string; readonly op: ComparisonOperator; readonly value?: import('../contribution').JsonValue }>>;
  readonly id: ContractId;
...
export interface SurfaceContributionDefinition extends PointContributionMetadata { readonly kind: 'surface'; readonly surfaceId: string; readonly initiallySelected?: boolean }
export interface TabContributionDefinition extends PointContributionMetadata { readonly kind: 'tab'; readonly tabId: string; readonly label: string; readonly surfaceId: string }
export interface ActionContributionDefinition extends PointContributionMetadata { readonly kind: 'action'; readonly actionId: string; readonly label: string; readonly visibleWhen?: readonly ActionCondition[]; readonly disabledWhen?: readonly ActionCondition[] }
export interface ActionDefinition { readonly id: string }
export type ExtensionContributionDefinition = SurfaceContributionDefinition | TabContributionDefinition | ActionContributionDefinition;
...
export interface HostContributionPolicyRequest { readonly contributorId: string; readonly ownerPluginId: string; readonly targetId: string; readonly kind: ExtensionKind; readonly contractMajor: number }
...
export function assertPoint(value: ExtensionPointDefinition): void {
  if (!value || Object.keys(value).some(k => !['id','kind','contractMajor','contextSchema','payloadSchema','profile','bindings','constraints'].includes(k)) || typeof value.id !== 'string' || !value.id.trim() || !['route','navigation','action','tab','surface'].includes(value.kind) || !Number.isSafeInteger(value.contractMajor) || value.contractMajor < 1) throw Error('Invalid Extension Point.');
  if (value.profile !== undefined) {
    if (typeof value.profile !== 'string' || !/^[^@\s]+@[1-9][0-9]*$/.test(value.profile) || value.contextSchema !== undefined || value.payloadSchema !== undefined) throw Error('Profile Points cannot redefine schemas.');
  } else {
...
export function assertExtensionContribution(value: ExtensionContributionDefinition): void {
  ...
  if (value.kind === 'action' ? typeof value.actionId !== 'string' || !value.actionId : typeof value.surfaceId !== 'string' || !value.surfaceId) throw Error('An execution id is required.');
  if (value.kind === 'tab' && (typeof value.tabId !== 'string' || !value.tabId)) throw Error('A Tab id is required.');
  if (value.kind === 'action') for (const conditions of [value.visibleWhen, value.disabledWhen]) {
    if (conditions !== undefined && (!Array.isArray(conditions) || conditions.length > 20)) throw Error('Invalid Action conditions.');
...
export function assertActionCondition(condition: ActionCondition): void {
  if (!condition || !['selectionCount','capability','trait','predicate'].includes(condition.source)) throw Error('Invalid Action condition.');
  const fields = condition.source === 'capability' ? ['source','id','available'] : condition.source === 'selectionCount' ? ['source','op','value'] : condition.source === 'trait' ? ['source','name','op','value','match'] : ['source','name','value','match'];
  if (Object.keys(condition).some(k => !fields.includes(k))) throw Error('Invalid Action condition.');
  if (condition.source === 'capability') {
    if (typeof condition.id !== 'string' || !/^[^@\s]+@[1-9][0-9]*$/.test(condition.id) || typeof condition.available !== 'boolean') throw Error('Invalid capability condition.');
  } else {
    if (condition.source !== 'selectionCount' && (typeof condition.name !== 'string' || !condition.name || !['all','any'].includes(condition.match))) throw Error('Invalid Ref condition.');
    if (condition.source === 'predicate' ? typeof condition.value !== 'boolean' : !['eq','ne','gt','gte','lt','lte','in','present'].includes(condition.op)) throw Error('Invalid comparison.');
  }
}
