...
export const uiOverlayBridgeContract: BridgeCapabilityContract = {
  ...
  actions: {
    ...Object.fromEntries(
      (['open', 'complete', 'cancel'] as const).map((action) => [
        action,
        {
          kind: 'request' as const,
          requestSchema: json,
          resultSchema: json,
          requiredPermissions: [UI_OVERLAY_PERMISSION],
          invoke(capability: unknown, payload: JsonValue, context: BridgeInvocationContext) {
            return (capability as UiOverlayCapability).invoke(action, payload, context);
          },
        },
      ]),
    ),
    observe: {
      ...
      eventSchema: json,
      requiredPermissions: [UI_OVERLAY_PERMISSION],
      open(capability, payload, context, emit) {
        return (capability as UiOverlayCapability).observe(payload, context, emit);
      },
    },
  },
};
...
export function createUiOverlayPlugin() {
  ...
  const plugin: import('../plugin.js').PluginDefinition = {
    ...
    version: '1.0.0',
    requires: [],
    provides: [UI_OVERLAY_CAPABILITY],
    activate({ capabilities }) {
      capabilities.register<UiOverlayCapability>(UI_OVERLAY_CAPABILITY, {
        invoke(action, payload, context) {
          if (!service) throw Error('UI_NOT_BOUND');
          return service.invoke(action, payload, context);
        },
        observe(payload, context, emit) {
          if (!service) throw Error('UI_NOT_BOUND');
          return service.observe(payload, context, emit);
        },
      });
    },
  };
  return {
    plugin,
    bind(value: UiOverlayCapability) {
      if (service) throw Error('UI_ALREADY_BOUND');
      service = value;
    },
  };
}
