...
export class UiError extends Error {
  constructor(
    ...
    readonly stage?: UiExecution['stage'],
  ) {
    super(code);
    this.name = 'UiError';
  }
...
export interface UiSizing {
  readonly mode: 'content-sized' | 'bounded';
...
export interface SlotInput {
  ...
  readonly selected?: readonly ContributionRef[];
  readonly sizing?: UiSizing;
}
...
export interface UiExecution {
  ...
  readonly stage?: 'artifact' | 'wujie-bootstrap' | 'handshake' | 'render' | 'bridge';
  readonly retryTarget?: string;
}
export interface UiRelation {
  readonly ref: ContributionRef;
  readonly definition: SurfaceContributionDefinition | TabContributionDefinition;
...
export interface SlotObservation {
  ...
  readonly input: {
    ...
    readonly error?: string;
    readonly lastAccepted: ContextSnapshot | null;
    readonly effective: ContextSnapshot | null;
  };
  readonly contributions: readonly {
    ...
    readonly execution?: UiExecution;
  }[];
}
export interface UiObservation {
  readonly streamId: string;
  readonly progress: number;
  readonly initialContext: ContextSnapshot | null;
  readonly context: ContextSnapshot | null;
  readonly overlayInput: JsonValue | null;
  readonly occurrences: readonly SlotObservation[];
}
...
export interface UiDriver {
  ...
  overlay(
    ...
    placement: unknown;
    dispose(): void;
    error(execution: UiExecution, retry: () => void): void;
  };
}
interface Scope {
  id: string;
  owner: string;
  kind: 'root' | 'contribution' | 'overlay';
  parent?: Scope;
  guardOccurrence?: string;
  active: boolean;
  children: Set<Scope>;
  attempt?: Attempt;
  surfaceId?: string;
...
interface Occurrence {
  ...
  anchor: unknown;
  input: SlotInput;
  lastAccepted: ContextSnapshot | null;
  effective: ContextSnapshot | null;
  error?: string;
  runtimeError?: string;
  executions: Map<string, Scope>;
  active: boolean;
}
interface Overlay {
  id: string;
  ...
  surfaceId: string;
  input: JsonValue;
  outcome: OverlayObservation['outcome'];
  execution: Scope;
  view: ReturnType<UiDriver['overlay']>;
}
...
export function createUiRuntime(options: UiRuntimeOptions) {
  ...
  const pointsById = new Map(
    points.map((point) => [uiKey(point.ownerPluginId, point.contribution.id), point]),
  );
  const surfacesById = new Map(
    surfaces.map((surface) => [uiKey(surface.ownerPluginId, surface.contribution.id), surface]),
  );
    ...
    queueMicrotask(() => {
      ...
      for (const listener of [...listeners]) {
        try {
          ...
          /* An observer cannot control execution. */
        }
      }
    });
  }
  const track = (task: Promise<void>) => {
    cleanups.add(task);
    void task.finally(() => cleanups.delete(task)).catch(() => undefined);
    return task;
  };
  function requireAttempt(attemptId: string): Attempt {
    ...
    for (let s: Scope | undefined = a.scope; s; s = s.parent)
      if (!s.active) throw new UiError('STALE_EXECUTION');
    return a;
  }
  function requireOccurrence(attemptId: string, occurrenceId: string): Occurrence {
    const a = requireAttempt(attemptId),
      o = occurrences.get(occurrenceId);
  ...
  function newScope(owner: string, kind: Scope['kind'], placement: unknown, parent?: Scope): Scope {
    ...
    if (scopes.size >= (options.maxScopes ?? 128) || depth >= (options.maxDepth ?? 8))
      throw new UiError('UI_RESOURCE_LIMIT');
    const s: Scope = {
      id: id('scope'),
      owner,
      kind,
  ...
  function start(
    ...
    const a: Attempt = {
      ...
      initial: frozenCopy(initial),
      pinned,
      occurrences: new Set(),
      execution: { phase: 'starting', attemptId: '' },
      task: Promise.resolve(),
    };
    a.execution = { phase: attached ? 'ready' : 'starting', attemptId: a.id };
    s.attempt = a;
    attempts.set(a.id, a);
    if (!attached)
      a.task = previous
        ...
        .then(async () => {
          if (!a.valid) return;
          try {
            a.mounted = await driver.mount({
              attemptId: a.id,
              ownerPluginId: s.owner,
            ...
            if (a.valid) {
              a.execution = { phase: 'ready', attemptId: a.id };
              s.changed?.();
              changed();
            }
          } catch (error) {
            if (a.valid) {
              try {
                options.onError?.(error, a.id);
              } catch {
                /* Diagnostics cannot keep a failed execution alive. */
              }
              failAttempt(
                a.id,
                error instanceof UiError ? error.code : 'SURFACE_FAILED',
                error instanceof UiError ? error.stage : undefined,
              );
            }
          }
        });
  ...
  function retryScope(s: Scope, target: string) {
    const old = s.attempt;
    if (!s.active || old?.id !== target || old.execution.phase !== 'failed')
      throw new UiError('STALE_RETRY');
    // This synchronous boundary is the Host's acceptance point, before slow cleanup/mount.
    const baseline = frozenCopy(s.context);
    const a = start(s, baseline, endAttempt(old), false, true);
    s.changed?.();
    return a.id;
  }
  function compileRelations(
    ...
    return extensions
      .filter(
        ({ contribution: c }) => c.point.ownerPluginId === pointOwner && c.point.id === point.id,
      )
      .sort((a, b) => comparePointContributions(point, a, b))
      .map(({ ownerPluginId, contribution: c }) => {
  ...
  const relationTable = new Map(
    points.map(({ ownerPluginId, contribution }) => [
      uiKey(ownerPluginId, contribution.id),
      frozenCopy(compileRelations(ownerPluginId, contribution)),
    ]),
  );
  function relations(
    owner: string,
    ...
    return relationTable.get(uiKey(owner, point.id)) ?? [];
  }
  function endOccurrence(o: Occurrence) {
    if (!o.active) return;
    o.active = false;
    occurrences.delete(o.id);
    o.attempt.occurrences.delete(o);
  ...
  function validateInput(input: SlotInput) {
    ...
    if (
      ...
      throw new UiError('INVALID_SLOT_INPUT');
    if (
      input.selected !== undefined &&
      (!Array.isArray(input.selected) ||
        input.selected.some(
          (r) => !r || typeof r.id !== 'string' || typeof r.ownerPluginId !== 'string',
        ))
    )
      throw new UiError('INVALID_SELECTION');
    ...
    if (size) {
      ...
      for (const key of ['minWidth', 'maxWidth', 'minHeight', 'maxHeight'] as const)
        if (size[key] !== undefined && (!Number.isFinite(size[key]) || size[key]! < 0))
          throw new UiError('INVALID_SIZING');
      if (
  ...
  function pointInput(
    ...
    const selected = relations(owner, point).filter(
      (r) =>
        r.availability === 'available' &&
        (input.selected === undefined ||
          input.selected.some(
            (ref) => uiKey(ref.ownerPluginId, ref.id) === uiKey(r.ref.ownerPluginId, r.ref.id),
          )),
    );
    if (
      point.constraints?.presentations &&
      !point.constraints.presentations.includes(point.kind === 'tab' ? 'tab' : 'inline')
    )
      throw new UiError('POINT_PRESENTATION_INVALID');
    const limits = point.constraints?.cardinality;
    if (limits && (selected.length < limits.min || selected.length > limits.max))
      throw new UiError('POINT_CARDINALITY_INVALID');
    ...
    if (bounds) {
      ...
      for (const name of ['Width', 'Height'] as const) {
        const min = `min${name}` as const,
          max = `max${name}` as const;
        if (
          (bounds[min] !== undefined && (size[min] ?? 0) < bounds[min]!) ||
          (bounds[max] !== undefined && (size[max] ?? Infinity) > bounds[max]!)
        )
          throw new UiError('POINT_SIZING_INVALID');
      }
  ...
  function reconcile(o: Occurrence, input: SlotInput) {
    ...
    const contextChanged =
      !o.lastAccepted ||
      keyChanged ||
      canonicalJson(o.input.context) !== canonicalJson(input.context);
    o.runtimeError = undefined;
    o.input = frozenCopy(input);
    if (matchesContext(o.point.contextSchema, input.context)) {
      o.error = undefined;
      if (contextChanged) {
        o.effective = frozenCopy({
          contextKey: input.contextKey,
          revision: ++revision,
          value: input.context,
        });
        o.lastAccepted = o.effective;
      }
    } else o.error = 'CONTEXT_INVALID';
    driver.visibility(o.anchor, input.hidden ?? false);
    const allowed = relations(o.attempt.scope.owner, o.point).filter(
      (r) => r.authorized && r.availability === 'available',
    );
    const selected = allowed.filter(
      (r) =>
        input.selected === undefined ||
        input.selected.some(
          (ref) => ref.ownerPluginId === r.ref.ownerPluginId && ref.id === r.ref.id,
        ),
    );
    for (const [key, s] of o.executions)
      if (!selected.some((r) => uiKey(r.ref.ownerPluginId, r.ref.id) === key)) {
        endScope(s);
        o.executions.delete(key);
      }
    if (o.effective)
      for (const r of selected) {
        const key = uiKey(r.ref.ownerPluginId, r.ref.id),
          existing = o.executions.get(key);
        if (existing) {
          existing.context = o.effective;
          try {
            driver.layout(
              existing.placement,
              input.sizing ?? { mode: 'content-sized' },
              selected.indexOf(r),
            );
          } catch {
            if (existing.attempt) failAttempt(existing.attempt.id);
          }
          continue;
        }
        ...
        try {
          ...
          s.guardOccurrence = o.id;
          driver.layout(
            cell.placement,
            input.sizing ?? { mode: 'content-sized' },
            selected.indexOf(r),
          );
          s.context = o.effective;
          s.surfaceId = r.definition.surfaceId;
          s.target = surface.contribution.target;
          o.executions.set(key, s);
          start(s, s.context);
        } catch (error) {
          if (s) endScope(s);
          else cell?.dispose();
  ...
  function slotSnapshot(o: Occurrence): SlotObservation {
    return {
      ...
      lifecycle: 'active',
      visibility: o.input.hidden ? 'hidden' : 'visible',
      input: {
        submittedKey: o.input.contextKey,
        acceptance: o.error ? 'rejected' : 'accepted',
        ...(o.error ? { error: o.error } : {}),
        lastAccepted: o.lastAccepted,
        effective: o.effective,
      },
      contributions: relations(o.attempt.scope.owner, o.point)
        .filter((r) => r.authorized)
        .map((r) => {
          const s = o.executions.get(uiKey(r.ref.ownerPluginId, r.ref.id));
          return {
            ref: r.ref,
            ...(r.definition.label ? { label: r.definition.label } : {}),
            ...(r.definition.kind === 'tab'
              ? { tabId: r.definition.tabId }
              : r.definition.initiallySelected === undefined
                ? {}
                : { initiallySelected: r.definition.initiallySelected }),
            availability: r.availability,
            ...(r.reason ? { reason: r.reason } : {}),
            selected:
              o.input.selected === undefined ||
              o.input.selected.some(
                (ref) => ref.ownerPluginId === r.ref.ownerPluginId && ref.id === r.ref.id,
              ),
            ...(s?.attempt ? { execution: s.attempt.execution } : {}),
          };
        }),
    };
  }
  function overlaySnapshot(overlay: Overlay): OverlayObservation {
    return {
      ...
      ...(overlay.execution.active && overlay.execution.attempt
        ? { execution: overlay.execution.attempt.execution }
        : {}),
    };
  }
  function finishOverlay(o: Overlay, outcome: OverlayObservation['outcome']) {
    ...
    o.outcome = frozenCopy(outcome);
    endScope(o.execution);
    o.view.dispose();
    changed();
    return o.outcome;
  }
  function ownedOverlay(attemptId: string, handle: string) {
    const a = requireAttempt(attemptId),
      o = overlays.get(handle);
    if (!o || o.owner !== a.scope) throw new UiError('OVERLAY_NOT_OWNED');
    return o;
  }
  const api = {
    attachOwner(owner: string, placement: unknown) {
      const s = newScope(owner, 'root', placement);
      const a = start(s, null, undefined, true);
      return { scopeId: s.id, attemptId: a.id, dispose: () => endScope(s) };
    },
    mountRoot(
      owner: string,
      surfaceId: string,
      ...
      return {
        ...
        get execution() {
          return s.attempt!.execution;
        },
        retry: () => retryScope(s, s.attempt!.id),
        dispose: () => endScope(s),
      };
    },
    identity(attemptId: string) {
      const a = requireAttempt(attemptId);
      return { ownerPluginId: a.scope.owner, scopeId: a.scope.id, signal: a.signal.signal };
    },
    ...
    updateSlot(attemptId: string, occurrenceId: string, input: SlotInput) {
      reconcile(requireOccurrence(attemptId, occurrenceId), input);
    },
    unmountSlot(attemptId: string, occurrenceId: string) {
      endOccurrence(requireOccurrence(attemptId, occurrenceId));
    },
    retry(attemptId: string, occurrenceId: string, target: string) {
      const o = requireOccurrence(attemptId, occurrenceId),
        s = [...o.executions.values()].find((s) => s.attempt?.id === target);
      if (!s) throw new UiError('STALE_RETRY');
      return retryScope(s, target);
    },
    snapshot(attemptId: string): UiObservation {
      ...
      return frozenCopy({
        streamId: a.id,
        progress,
        initialContext: a.pinned ? a.initial : a.scope.context,
        context: a.scope.context,
        overlayInput: a.scope.overlay?.input ?? null,
        occurrences: [...a.occurrences].map(slotSnapshot),
      });
    },
    ...
    openOverlay(
      ...
      if (a.scope.overlays.size >= (options.maxOverlaysPerScope ?? 32))
        throw new UiError('UI_RESOURCE_LIMIT');
      ...
      const view = driver.overlay(handle, presentation, () => {
        const o = overlays.get(handle);
        if (o) finishOverlay(o, { state: 'cancelled' });
      });
      let execution: Scope;
      try {
        execution = newScope(a.scope.owner, 'overlay', view.placement, a.scope);
      } catch (error) {
        view.dispose();
        throw error;
      }
      ...
      execution.surfaceId = surfaceId;
      execution.target = surface.contribution.target;
      execution.overlay = o;
      execution.changed = () => {
        if (execution.attempt && o.outcome.state === 'pending')
          view.error(execution.attempt.execution, () => {
            if (execution.attempt) retryScope(execution, execution.attempt.id);
          });
      };
      overlays.set(handle, o);
      a.scope.overlays.add(o);
    ...
    completeOverlay(attemptId: string, result: JsonValue) {
      assertUiJson(result);
      const a = requireAttempt(attemptId);
      if (!a.scope.overlay) throw new UiError('NOT_OVERLAY_EXECUTION');
      return finishOverlay(a.scope.overlay, { state: 'completed', result });
    },
    cancelOverlay(attemptId: string, handle: string) {
      return finishOverlay(ownedOverlay(attemptId, handle), { state: 'cancelled' });
    },
    inspect() {
      const relationFacts = extensions.map(({ ownerPluginId, contribution }) => {
        const point = pointsById.get(
          uiKey(contribution.point.ownerPluginId, contribution.point.id),
        );
        const relation =
          point &&
          relations(point.ownerPluginId, point.contribution).find(
            (r) => r.ref.ownerPluginId === ownerPluginId && r.ref.id === contribution.id,
          );
        return {
          ref: { ownerPluginId, id: contribution.id },
          point: contribution.point,
          availability: relation?.availability ?? 'unavailable',
          reason: relation?.reason ?? (point ? undefined : 'POINT_MISSING'),
        };
      });
      return frozenCopy({
        progress,
        relations: relationFacts,
        scopes: [...scopes.values()].map((s) => ({
          id: s.id,
          owner: s.owner,
          kind: s.kind,
          parentId: s.parent?.id,
          guardOccurrence: s.guardOccurrence,
          attemptId: s.attempt?.id,
          execution: s.attempt?.execution,
        })),
        ...
        occurrences: [...occurrences.values()].map((o) => ({ id: o.id, attemptId: o.attempt.id })),
        overlays: overlays.size,
      });
    },
    async settled() {
      while (cleanups.size) await Promise.allSettled([...cleanups]);
    },
    async dispose() {
      closed = true;
      for (const s of [...scopes.values()]) if (!s.parent) endScope(s);
      await api.settled();
      listeners.clear();
    },
  };
