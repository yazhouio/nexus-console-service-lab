...
export function Slot({ feedback, panel, ...input }: SlotProps) {
  ...
  return <section data-nexus-slot={input.id} hidden={input.hidden}>
    ...
    <div ref={anchorRef} id={panel?.id} role={panel ? 'tabpanel' : undefined} aria-label={panel?.label} tabIndex={panel ? 0 : undefined} />
  </section>;
}

const RoutePresentation = createContext<{ context?: import('./contribution').RouteContext; outlet: ReactNode } | undefined>(undefined);
/** Host adapter boundary. Plugins consume RouteOutlet/useRouteContext without importing the Host. */
export function RoutePresentationProvider({ context, outlet, children }: { context?: import('./contribution').RouteContext; outlet: ReactNode; children: ReactNode }) {
  return <RoutePresentation value={{ context, outlet }}>{children}</RoutePresentation>;
}
export function RouteOutlet() { return useContext(RoutePresentation)?.outlet ?? null; }
...
const RouteLinks = createContext<((props: RouteLinkProps, client: UiClient) => ReactNode) | undefined>(undefined);
export function RouteLinkProvider({ renderLink, children }: { renderLink: (props: RouteLinkProps, client: UiClient) => ReactNode; children: ReactNode }) {
...
export function RouteLink(props: RouteLinkProps) {
  const render = useContext(RouteLinks), client = useUiClient();
  if (!render) throw Error('No Route Link adapter.');
  return render(props, client);
}

export function useCapabilitySubscription<T>(capability: `${string}@${number}`, action: string, payload: import('./contribution').JsonValue = null): { value: T | undefined; error: Error | undefined } {
  const client = useUiClient(), key = JSON.stringify(payload);
  const [state, setState] = useState<{ value: T | undefined; error: Error | undefined }>({ value: undefined, error: undefined });
  useEffect(() => {
    let active = true, stop: (() => void) | undefined;
    setState({ value: undefined, error: undefined });
    void client.observe<T>(capability, action, JSON.parse(key), value => { if (active) setState({ value, error: undefined }); }).then(dispose => { if (active) stop = dispose; else dispose(); }).catch(error => { if (active) setState({ value: undefined, error }); });
...
export function ActionMenu({ point, context }: { point: import('./ui/definitions').ExtensionPointRef; context: import('./contribution').JsonValue }) {
  ...
  useEffect(() => {
    ...
    void client.actions.query(input.point, input.context).then(value => { if (current) setChoices(value); }).catch(() => { if (current) setStatus('Actions unavailable'); });
    return () => { current = false; };
  }, [client, key, status]);
  const run = async (ref: import('./ui/definitions').ContributionRef) => {
    setStatus('Starting action…'); setResult(undefined);
    try {
      const invocation = await client.actions.start(point, ref, context);
      if (active.current) setPending(invocation);
      const result = await invocation.result;
      if (active.current) { setStatus(`Action ${result.state}`); if (result.state === 'succeeded') setResult(result.result); }
