...
const identity = (
  item: OwnedContribution<{ id: string }>,
  kind: ContributionIdentity['kind'],
): ContributionIdentity => ({
  ownerPluginId: item.ownerPluginId,
  kind,
  contributionId: item.contribution.id,
});
const stable = <T extends OwnedContribution<{ id: string }>>(a: T, b: T) =>
...
export function createRouteModel(input: {
  ...
  const points = new Map(
    (input.points ?? []).map((p) => [uiKey(p.ownerPluginId, p.contribution.id), p.contribution]),
  );
  function admission(
    item: OwnedContribution<RouteContribution | NavigationContribution>,
    kind: 'route' | 'navigation',
    expected?: ExtensionPointRef,
    expectedOwner?: string,
  ): ContributionDiagnostic | undefined {
    const ref = item.contribution.point;
    if (!expected || !ref) return { code: 'POINT_MISSING' };
    if (expectedOwner !== undefined && expected.ownerPluginId !== expectedOwner)
      return { code: 'POINT_TARGET_MISMATCH' };
    ...
    if (ref.ownerPluginId !== expected.ownerPluginId || ref.id !== expected.id)
      return { code: 'POINT_TARGET_MISMATCH' };
    const result = admitContribution(
      item.ownerPluginId,
      { ...item.contribution, point: ref, kind },
      point,
      input.policy ?? (() => false),
    );
    return result.reason ? { code: result.reason as ContributionDiagnostic['code'] } : undefined;
  }
  const routes: ResolvedRoute[] = [...input.routes]
    .sort(stable)
    .map((item) => ({ ...item, state: 'AVAILABLE', diagnostics: [], ancestry: [], ownParams: [] }));
  ...
  function inheritFailure(route: ResolvedRoute, parent: ResolvedRoute) {
    fail(route, 'UNREACHABLE', {
      code: parent.state === 'QUARANTINED' ? 'PARENT_QUARANTINED' : 'PARENT_UNREACHABLE',
      related: [identity(parent, 'route')],
      cause: parent.diagnostics[0],
    });
  }
  function compile(route: ResolvedRoute): void {
    ...
    if (cycleAt !== -1) {
      ...
      for (const member of cycle) {
        fail(member, 'QUARANTINED', {
          code: 'PARENT_CYCLE',
          related: cycle.map((r) => identity(r, 'route')),
        });
    ...
    if (!done.has(id)) {
      ...
      if (local && (parentRouteId === undefined || parent?.space)) {
        route.space = makePath(
          [...(parent?.space?.segments ?? []), ...local.segments],
          local.splat,
        );
      }
      if (parentRouteId !== undefined && !parent)
        fail(route, 'UNREACHABLE', { code: 'PARENT_MISSING' });
      else if (parent && parent.state !== 'AVAILABLE') inheritFailure(route, parent);
      else if (!local) fail(route, 'QUARANTINED', { code: 'INVALID_PATH' });
      else if (parent?.space?.splat)
        fail(route, 'QUARANTINED', {
          code: 'PARENT_WILDCARD',
          related: [identity(parent, 'route')],
        });
      else if (
        parent &&
        (parent.contribution.target.kind !== 'builtin' || !parent.contribution.target.routeLayout)
      )
        fail(route, 'QUARANTINED', {
          code: 'PARENT_NOT_LAYOUT',
          related: [identity(parent, 'route')],
        });
      else if (
        parent &&
        parent.ownerPluginId !== route.ownerPluginId &&
        !parent.contribution.childPoint
      )
        fail(route, 'QUARANTINED', {
          code: 'EXTENSION_POINT_CLOSED',
          related: [identity(parent, 'route')],
        });
      else {
        const params = route.space!.segments.flatMap((s) => (s.kind === 'param' ? [s.name] : []));
        if (new Set(params).size !== params.length)
          fail(route, 'QUARANTINED', { code: 'DUPLICATE_PARAMETER' });
        else {
          const denied = admission(
            route,
            'route',
            parent ? parent.contribution.childPoint : input.rootRoutePoint,
  ...
  for (let i = 0; i < candidates.length; i++)
    for (let j = i + 1; j < candidates.length; j++) {
      const a = candidates[i],
        b = candidates[j];
      if (a.ancestry.includes(b.contribution.id) || b.ancestry.includes(a.contribution.id))
        continue;
      const relation = relationship(a.space!, b.space!);
      if (relation.kind === 'DISJOINT') continue;
      if (a.ownerPluginId === b.ownerPluginId && ['CONTAINS', 'WITHIN'].includes(relation.kind))
        continue;
  ...
  for (const route of [...routes].sort((a, b) => a.ancestry.length - b.ancestry.length)) {
    ...
    if (
      ...
      !route.diagnostics.some((d) => d.code === 'PARENT_CYCLE')
    )
      inheritFailure(route, parent);
  }
  function toRouteObjects(
    render?: (route: ResolvedRoute) => RouteObject['element'],
    includeUnavailable = false,
  ): RouteObject[] {
    const eligible = routes.filter(
      (r) => r.space && (includeUnavailable || r.state === 'AVAILABLE'),
    );
    const parameterNames = new Set(
      routes.flatMap(
        (r) => r.space?.segments.flatMap((s) => (s.kind === 'param' ? [s.name] : [])) ?? [],
      ),
    );
      ...
      const segments =
        route.state === 'AVAILABLE'
          ...
          : local.segments.map((segment) => {
              ...
              let name: string;
              do {
                name = `__nexus_unavailable_${placeholderIndex++}`;
              } while (parameterNames.has(name));
    ...
    const project = (parentId?: string): RouteObject[] =>
      eligible
        .filter((r) => r.contribution.parentRouteId === parentId)
        .map((r) => ({
          id: r.contribution.id,
          path: projectedPath(r),
          caseSensitive: false,
          element: render?.(r),
          children: project(r.contribution.id),
        }));
    return project();
  }
  function resolve(url: string): UrlResolution {
    const location = new URL(
      url.startsWith('/') ? `http://host.invalid${url}` : url,
      'http://host.invalid',
    );
    try {
      location.pathname.split('/').forEach((segment) => decodeURIComponent(segment));
    } catch {
    ...
    const blocked = routes.filter(
      (r) =>
        r.state !== 'AVAILABLE' &&
        r.space &&
        matchRoutes([{ path: r.space.path, caseSensitive: false }], location.pathname),
    );
    if (blocked.length) {
      const ancestors = blocked[0].ancestry.slice(0, -1).filter((id) => {
        ...
        return (
          parent.state === 'AVAILABLE' &&
          !parent.space?.splat &&
          parent.contribution.target.kind === 'builtin' &&
          parent.contribution.target.routeLayout &&
          blocked.every((r) => r.ancestry.slice(0, -1).includes(id))
        );
      });
      const parent = byId.get(ancestors.at(-1) ?? '');
      const matchedParent = parent
        ? matchRoutes(
            [{ path: parent.space!.path.replace(/\/$/, '') + '/*' }],
            location.pathname,
          )?.[0]
        : undefined;
      const params = Object.fromEntries(
        Object.entries(matchedParent?.params ?? {}).filter(
          (p): p is [string, string] => p[0] !== '*' && p[1] !== undefined,
        ),
      );
      return {
        state: 'CONFLICT',
        ancestors,
        params,
        diagnostics: blocked.flatMap((r) => r.diagnostics),
      };
    }
    ...
    const params = Object.fromEntries(
      Object.entries(leaf.params).filter((p): p is [string, string] => p[1] !== undefined),
    );
    return {
      state: 'MATCHED',
      routeId: leaf.route.id!,
  ...
  const items: ResolvedNavigation[] = [...input.navigation]
    .sort(stable)
    .map((item) => ({ ...item, state: 'AVAILABLE', diagnostics: [], children: [] }));
  ...
  function compileNavigation(item: ResolvedNavigation): void {
    ...
    if (cycleAt !== -1) {
      ...
      for (const member of cycle) {
        navFail(member, 'QUARANTINED', {
          code: 'PARENT_CYCLE',
          related: cycle.map((n) => identity(n, 'navigation')),
        });
        navDone.add(member.contribution.id);
      }
      return;
    }
    navStack.push(id);
    ...
    if (!navDone.has(id)) {
      if (parentId !== undefined && !parent)
        navFail(item, 'UNREACHABLE', { code: 'PARENT_MISSING' });
      else if (parent && parent.state !== 'AVAILABLE')
        navFail(item, 'UNREACHABLE', {
          code: parent.state === 'QUARANTINED' ? 'PARENT_QUARANTINED' : 'PARENT_UNREACHABLE',
          related: [identity(parent, 'navigation')],
          cause: parent.diagnostics[0],
        });
      else if (
        parent &&
        ...
        navFail(item, 'QUARANTINED', {
          code: 'EXTENSION_POINT_CLOSED',
          related: [identity(parent, 'navigation')],
        });
      else {
        const rootPoint = input.navigationRootPoints?.find(
          (p) =>
            p.ownerPluginId === item.contribution.point?.ownerPluginId &&
            p.id === item.contribution.point?.id,
        );
        const denied = admission(
          item,
          'navigation',
          parent ? parent.contribution.childPoint : rootPoint,
          parent?.ownerPluginId,
        );
  ...
  const navigation = items.filter(
    (item) => item.state === 'AVAILABLE' && !item.contribution.parentId,
  );
  for (const item of items)
    if (item.state === 'AVAILABLE' && item.contribution.parentId)
      navigationById.get(item.contribution.parentId)!.children.push(item);

  const sortSiblings = (siblings: ResolvedNavigation[]) =>
    siblings.sort((a, b) => {
      const point = a.contribution.point;
      return comparePointContributions(
        point ? points.get(uiKey(point.ownerPluginId, point.id)) : undefined,
        a,
        b,
      );
    });
  ...
  function destination(id: string, url: string): NavigationDestination {
    ...
    if (target.state !== 'AVAILABLE')
      return {
        ...
        diagnostic: {
          ...
          related: [identity(target, 'route')],
          cause: target.diagnostics[0],
        },
      };
    ...
    if (current.state !== 'NOT_FOUND') {
      ...
      for (const [index, ancestorId] of target.ancestry.entries()) {
        ...
        for (const name of byId.get(ancestorId)!.ownParams)
          if (Object.hasOwn(bindings, name)) params[name] = bindings[name];
      }
    }
    const missing = target.space!.segments.flatMap((s) =>
      s.kind === 'param' && !Object.hasOwn(params, s.name) ? [s.name] : [],
    );
    if (missing.length)
      return {
        state: 'DISABLED',
        diagnostic: { code: 'TARGET_PARAMS_MISSING', missingParams: missing },
      };
    const href =
      '/' +
      target
        .space!.segments.map((s) =>
          s.kind === 'literal' ? s.value : encodeURIComponent(params[s.name]),
        )
        .join('/');
    const resolved = resolve(href);
    if (resolved.state !== 'MATCHED' || !resolved.chain.includes(targetId))
      return { state: 'DISABLED', diagnostic: { code: 'TARGET_SHADOWED' } };
  ...
  function paramsSchema(routeId: string): ContextSchema {
    ...
    return frozenCopy({
      ...
      properties: Object.fromEntries([
        ...names.map((name) => [name, { type: 'string', minLength: 1, maxLength: 1024 }]),
        ...(route.space.splat ? [['*', { type: 'string', maxLength: 2048 }]] : []),
      ]),
      required: names,
  ...
  function pathFor(routeId: string, params: Readonly<Record<string, string>>): string {
    ...
    const path =
      '/' +
      [
        ...route.space.segments.map((s) =>
          s.kind === 'literal' ? s.value : encodeURIComponent(params[s.name]),
        ),
        ...(route.space.splat && params['*'] ? params['*'].split('/').map(encodeURIComponent) : []),
      ].join('/');
    const resolved = resolve(path);
    if (
      resolved.state !== 'MATCHED' ||
      !resolved.chain.includes(routeId) ||
      Object.entries(params).some(([name, value]) => resolved.context.params[name] !== value)
    )
      throw Error('ROUTE_TARGET_UNREACHABLE');
    return path;
  }
  function listHostContributions(url?: string): readonly HostContributionFact[] {
    return [
      ...routes.map((r) => ({
        ...identity(r, 'route'),
        state: r.state,
        declaredPath: r.contribution.path,
        fullPath: r.space?.path,
        parentId: r.contribution.parentRouteId,
        diagnostics: r.diagnostics,
      })),
      ...items.map((item) => {
        const dest = url === undefined ? undefined : destination(item.contribution.id, url);
        return {
          ...identity(item, 'navigation'),
          state: item.state,
          parentId: item.contribution.parentId,
          diagnostics: item.diagnostics,
          ...(item.state === 'AVAILABLE' && dest?.state === 'DISABLED'
            ? { navigationDiagnostic: dest.diagnostic }
            : {}),
        };
      }),
    ];
  ...
  for (const route of routes) {
    Object.assign(route, frozenCopy(route));
    Object.freeze(route);
  }
  for (const item of items) {
    Object.assign(item, frozenCopy(item));
    Object.freeze(item);
  }
