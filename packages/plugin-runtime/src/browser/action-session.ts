...
export function createActionExecutionChannel(signal: AbortSignal) {
  ...
  let started = false, readyReceived = false, completed = false, closed = false, lastRequest = 0;
  let readyResolve!: () => void, readyReject!: (error: Error) => void;
  let resultResolve!: (value: JsonValue) => void, resultReject!: (error: Error) => void;
  const ready = new Promise<void>((resolve, reject) => { readyResolve = resolve; readyReject = reject; });
  const result = new Promise<JsonValue>((resolve, reject) => { resultResolve = resolve; resultReject = reject; });
  ...
  const dispose = () => {
    ...
    readyReject(Error('ACTION_CONNECTION_CLOSED')); resultReject(Error('ACTION_CONNECTION_CLOSED'));
  };
  const cancel = () => { send?.({ type: 'action:cancel' }); dispose(); };
  ...
  const control: CreateUiControl = sender => {
    ...
    return { dispatch(value) {
      ...
      try {
        assertUiJson(value);
        const message = value as { requestId: number; action: string; payload: { ok: boolean; result?: JsonValue } };
        if (!Number.isSafeInteger(message.requestId) || message.requestId <= lastRequest) throw Error('ACTION_PROTOCOL_INVALID');
        lastRequest = message.requestId;
        if (message.action === 'action.ready' && !readyReceived && !started) { readyReceived = true; clearTimeout(timer); readyResolve(); }
        else if (message.action === 'action.complete' && started && !completed) {
          completed = true;
          if (message.payload?.ok === true) { assertUiJson(message.payload.result); resultResolve(message.payload.result); }
          else resultReject(Error('ACTION_FAILED'));
        } else throw Error('ACTION_PROTOCOL_INVALID');
      } catch { dispose(); }
    }, dispose };
  };
  return { control, ready, invoke() {
    if (closed || started || !readyReceived) throw Error('ACTION_ALREADY_FINISHED');
    started = true; send?.({ type: 'action:execute' }); return result;
  }, cancel, dispose };
}
