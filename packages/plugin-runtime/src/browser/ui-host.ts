...
export function createUiHost(options: UiHostOptions) {
  ...
  function controlReady(attemptId: string, signal: AbortSignal) {
    const promise = new Promise<void>((resolve, reject) => {
      const cleanup = () => { clearTimeout(timeout); readyCallbacks.delete(attemptId); signal.removeEventListener('abort', abort); };
      const abort = () => { cleanup(); reject(new UiError('STALE_EXECUTION')); };
      const timeout = setTimeout(() => { cleanup(); reject(new UiError('UI_READY_TIMEOUT')); }, 10_000);
      readyCallbacks.set(attemptId, () => { cleanup(); resolve(); });
  ...
  function belongs(element: Node, root: ParentNode, attemptId: string) {
    for (let node: Node | null = element; node; node = physicalParent.call(node)) {
      const owner = rootOwners.get(node); if (owner && owner !== attemptId) return false;
  ...
  function resolveAnchor(attemptId: string, token: string): HTMLElement {
    ...
    const matches = [...root.querySelectorAll<HTMLElement>('[data-nexus-slot-anchor]')].filter(el => el.getAttribute('data-nexus-slot-anchor') === token && belongs(el, root, attemptId));
  ...
  function channel(attemptId: string) {
    return (send: (value: unknown) => void) => {
      const owned = new Map<string, HTMLElement>(); anchors.set(attemptId, owned);
      let resolved: HTMLElement | undefined;
      const observer = new MutationObserver(() => {
        const root = getRoot(attemptId);
        for (const [occurrenceId, element] of owned) if (!root || !element.isConnected || !belongs(element, root, attemptId)) {
          try { core.unmountSlot(attemptId, occurrenceId); } catch { /* Already invalidated. */ }
      ...
      const ingress = delegate(value => {
        ...
        if (reply.type === 'ui:response' && reply.ok && typeof reply.result === 'string' && resolved) {
          ...
          const root = getRoot(attemptId); if (root) observer.observe(root, { childList: true, subtree: true });
        }
        send(value);
        try { const snapshot = core.snapshot(attemptId); for (const key of owned.keys()) if (!snapshot.occurrences.some(o => o.occurrenceId === key)) owned.delete(key); } catch { /* Ended Attempt. */ }
      });
      return { dispatch(value: unknown) { resolved = undefined; ingress.dispatch(value); resolved = undefined; }, dispose() { observer.disconnect(); ingress.dispose(); anchors.delete(attemptId); } };
    };
  }
  function localClient(attemptId: string): { client: UiClient; dispose(): void; settled(): Promise<void> } {
    ...
    const dispose = () => {
      ...
      client.dispose(); session.dispose(); ports.port2.close(); bindings.delete(attemptId);
      const cleanup = session.settled(); localCleanups.add(cleanup);
      void cleanup.finally(() => localCleanups.delete(cleanup));
    };
    identity.signal.addEventListener('abort', dispose, { once: true }); return { client, dispose, settled: () => session.settled() };
  }
  function applyLayout(element: HTMLElement, sizing: UiSizing, order: number) {
    Object.assign(element.style, { position: 'relative', isolation: 'isolate', contain: 'layout paint', overflow: 'auto', flex: 'none', order: String(order), width: '100%', minWidth: `${sizing.minWidth ?? 0}px`, maxWidth: sizing.maxWidth === undefined ? '100%' : `${sizing.maxWidth}px`, minHeight: `${sizing.minHeight ?? 0}px`, maxHeight: sizing.maxHeight === undefined ? 'none' : `${sizing.maxHeight}px`, height: sizing.mode === 'bounded' ? `${Math.min(sizing.maxHeight ?? 320, Math.max(sizing.minHeight ?? 0, 320))}px` : 'auto' });
    element.dataset.nexusSizing = sizing.mode;
  }
  const core = createUiRuntime({ registry: options.runtime.contributions, policy: options.policy, onError: options.onError,
    canOverlay: owner => permissions(owner).includes(UI_OVERLAY_PERMISSION) && (candidate(owner)?.requires.includes(UI_OVERLAY_CAPABILITY) ?? false),
    driver: {
      allocate(anchor, id, sizing) {
        const parent = anchor as HTMLElement;
        Object.assign(parent.style, { display: 'flex', flexDirection: 'column', minWidth: '0', maxWidth: '100%', overflow: 'auto', contain: 'layout paint' });
        const element = document.createElement('div'); element.dataset.nexusContribution = id; applyLayout(element, sizing, 0); parent.append(element);
        return { placement: element, dispose: () => element.remove() };
      },
      layout(placement, sizing, order) { applyLayout(placement as HTMLElement, sizing, order); },
      ...
      async mount(input) {
        ...
        const mounting = options.restrictedAdapter.mount({ pluginId: input.ownerPluginId, surfaceId: input.surfaceId, container,
          mountPointId: meta?.mountPointId ?? input.attemptId, routeContext: meta?.routeContext,
          layout: input.target.layout, initialParameters: input.target.initialParameters, signal: input.signal,
          createUiControl: channel(input.attemptId), onExecutionFailure: issue => core.failAttempt(input.attemptId, issue.code, issue.stage),
        });
        const record = options.restrictedAdapter.listInstances().find(i => i.identity.mountPointId === (meta?.mountPointId ?? input.attemptId));
        if (record) bindings.set(record.identity.surfaceInstanceId, input.attemptId);
        input.signal.addEventListener('abort', () => { if (record) bindings.delete(record.identity.surfaceInstanceId); }, { once: true });
        let mounted;
        try { mounted = await mounting; await ready; } catch (error) {
          if (record) { bindings.delete(record.identity.surfaceInstanceId); await options.restrictedAdapter.unmount(record.identity.surfaceInstanceId); }
          throw error;
        }
        const root = getRoot(input.attemptId);
        const style = document.createElement('style');
        style.textContent = 'html,body{min-height:0!important;margin:0!important;width:100%!important}'; root?.appendChild(style);
        ...
        const resize = new ResizeObserver(() => {
          if (!input.signal.aborted && content && container.dataset.nexusSizing === 'content-sized') {
            const app = container.querySelector<HTMLElement>('wujie-app'); if (app) app.style.height = `${content.getBoundingClientRect().height}px`;
          }
        });
        if (content) resize.observe(content);
        return { async dispose() { resize.disconnect(); disposeBindings(); bindings.delete(mounted.identity.surfaceInstanceId); await mounted.unmount(); } };
      },
      overlay(handle, presentation, close) {
        ...
        dialog.append(closeButton, feedback, container); document.body.append(dialog); dialog.addEventListener('cancel', event => { event.preventDefault(); close(); }); dialog.showModal();
        return { placement: container, dispose() { if (!dialog.isConnected) return; dialog.close(); dialog.remove(); if (previous?.isConnected) previous.focus(); }, error(execution, retry) { feedback.replaceChildren(); if (execution.phase === 'failed') { const message = document.createElement('p'); message.setAttribute('role', 'alert'); message.textContent = 'Unable to display this view.'; const button = document.createElement('button'); button.textContent = 'Retry'; button.onclick = retry; feedback.append(message, button); } } };
      },
    },
  });
  const overlayCapability: UiOverlayCapability = {
    invoke(action, payload, context) {
      const attemptId = bindings.get(context.surfaceInstanceId); if (!attemptId) throw new UiError('STALE_EXECUTION');
      const p = payload as any;
      if (action === 'open') return core.openOverlay(attemptId, p?.surfaceId, p?.input, p?.presentation);
      if (action === 'complete') { core.completeOverlay(attemptId, payload); return null; }
      core.cancelOverlay(attemptId, p?.handle); return null;
    },
    observe(payload, context, emit) {
      const attemptId = bindings.get(context.surfaceInstanceId); if (!attemptId) throw new UiError('STALE_EXECUTION');
      const handle = payload === null ? undefined : (payload as any).handle;
      const read = () => JSON.parse(JSON.stringify(core.overlaySnapshot(attemptId, handle))) as JsonValue;
      const snapshot = read(); const dispose = core.subscribe(() => { try { emit(read()); } catch { /* Scope invalidation ends this observer. */ } });
  ...
  return {
    ...
    attachOwner(owner: string, container: HTMLElement) {
      ...
      return { client: local.client, dispose() { root.dispose(); local.dispose(); roots.delete(root.attemptId); } };
    },
    mountRoot(owner: string, surfaceId: string, target: HostRenderTarget, container: HTMLElement, mountPointId: string, routeContext?: RouteContext) {
      metadata.set(container, { mountPointId, routeContext });
      const root = core.mountRoot(owner, surfaceId, target, container, rootCleanups.get(mountPointId));
      return {
        scopeId: root.scopeId, get attemptId() { return root.attemptId; }, get execution() { return root.execution; }, retry: root.retry,
        dispose() {
          root.dispose();
          const cleanup = core.settled(); rootCleanups.set(mountPointId, cleanup);
          void cleanup.then(() => { if (rootCleanups.get(mountPointId) === cleanup) rootCleanups.delete(mountPointId); });
        },
      };
    },
