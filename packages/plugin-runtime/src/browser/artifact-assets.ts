...
export function createArtifactAssets(timeoutMs = 15_000) {
  ...
  function retain(root: StyleRoot, url: string) {
    ...
    if (!entry) {
      ...
      try {
        (root.nodeType === 9 ? (root as Document).head : root).appendChild(link);
      } catch (error) {
        fail(`CSS_INSERT_ERROR ${String(error)}`);
      }
    }
    ++entry.references;
    let released = false;
    return {
      ...
      release() {
        if (released) return;
        released = true;
        if (--entry.references === 0) entry.remove();
      },
    };
  }
  return {
        ...
        ...new Set(
          urls.map((url) => {
            try {
              ...
              throw new Error(`CSS_URL_INVALID: ${url}`);
            }
          }),
        ),
      ];
