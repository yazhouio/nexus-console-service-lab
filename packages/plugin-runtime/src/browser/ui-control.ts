...
export function createUiControl(core: UiRuntime, attemptId: string, resolveAnchor: (anchor: string) => unknown, onReady?: () => void, actions?: ActionRuntime): CreateUiControl {
  return send => {
    ...
    return {
        ...
        try {
          ...
          switch (v.action) {
            case 'action.query':
              ...
              result = JSON.parse(JSON.stringify(actions.query(identity.ownerPluginId, p.point, p.context))); break;
            case 'action.start': {
              if (!actions || invocations.size >= 128) throw new UiError('ACTION_RESOURCE_LIMIT');
              const invocation = actions.invoke(identity.ownerPluginId, p.point, p.ref, p.context, { payload: p.payload, timeoutMs: p.timeoutMs, signal: identity.signal });
              invocations.set(invocation.invocationId, invocation); result = invocation.invocationId; break;
            }
            case 'action.wait': {
              const invocation = invocations.get(p.invocationId); if (!invocation) throw new UiError('ACTION_NOT_OWNED');
              void invocation.result.then(outcome => { invocations.delete(p.invocationId); if (active) send({ type: 'ui:response', requestId, ok: true, result: outcome }); });
              return;
            }
            case 'action.cancel': {
              const invocation = invocations.get(p.invocationId); if (!invocation) throw new UiError('ACTION_NOT_OWNED');
              invocation.cancel(); break;
            }
      ...
      dispose() { active = false; stop(); for (const invocation of invocations.values()) invocation.cancel(); invocations.clear(); },
    };
  };
}
