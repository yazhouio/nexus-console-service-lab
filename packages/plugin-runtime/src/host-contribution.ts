...
export type ContributionDiagnosticCode =
  | 'POLICY_ERROR' | 'POINT_MISSING' | 'POINT_TARGET_MISMATCH' | 'CONTRACT_MISMATCH' | 'KIND_MISMATCH' | 'PROFILE_ASSERTION_FAILED' | 'REF_CONTRACT_ASSERTION_FAILED' | 'GROUP_NOT_ALLOWED' | 'ORDER_OUT_OF_RANGE'
  | 'INVALID_PATH' | 'DUPLICATE_PARAMETER' | 'PARENT_MISSING' | 'PARENT_QUARANTINED'
  | 'PARENT_UNREACHABLE' | 'PARENT_NOT_LAYOUT' | 'PARENT_WILDCARD' | 'PARENT_CYCLE'
  | 'EXTENSION_POINT_CLOSED' | 'POLICY_DENIED' | 'ROUTE_CONFLICT'
  | 'TARGET_MISSING' | 'TARGET_UNAVAILABLE' | 'TARGET_PARAMS_MISSING' | 'TARGET_SHADOWED';
