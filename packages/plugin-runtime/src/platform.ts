...
interface PlatformService {
  ...
  ): { snapshot: JsonValue; dispose(): void };
}
...
const boundedJson: RuntimeSchema<JsonValue> = {
  parse(value) {
    assertUiJson(value);
    return value;
  },
...
function request(schema: ContextSchema): RuntimeSchema<JsonValue> {
  return {
    parse(value) {
      assertUiJson(value);
      if (!matchesContext(schema, value)) throw Error('INVALID_PLATFORM_REQUEST');
      return value;
    },
  };
}
...
): ContextSchema => ({ type: 'object', properties, required, additionalProperties: false });
const unarySchemas: Readonly<
  Record<PlatformCapabilityId, Readonly<Record<string, RuntimeSchema<JsonValue>>>>
> = {
  'routes.query@1': { list: empty, current: empty, navigation: empty },
  'routes.navigate@1': {
    navigate: request(object({ routeId: id, params: { type: 'object', maxProperties: 32 } })),
  },
  'plugins.query@1': { list: empty },
  'plugins.manage@1': {
    install: request(object({ id, version: id })),
    selectVersion: request(object({ id, version: id })),
    setEnabled: request(object({ id, enabled: { type: 'boolean' } })),
    uninstall: request(object({ id })),
  },
  'diagnostics.query@1': { get: empty },
  'diagnostics.export@1': { export: empty },
  'audit.query@1': { list: empty },
};
export const platformBridgeContracts: readonly BridgeCapabilityContract[] = frozenCopy(
  PLATFORM_CAPABILITIES.map((capability) => ({
    id: capability,
    actions: {
      ...Object.fromEntries(
        Object.entries(unarySchemas[capability]).map(([action, requestSchema]) => [
          action,
          {
            kind: 'request' as const,
            requestSchema,
            resultSchema: boundedJson,
            requiredPermissions: [capability.split('@')[0]],
            invoke(value: unknown, payload: JsonValue, context: BridgeInvocationContext) {
              return (value as PlatformService).invoke(action, payload, context);
            },
          },
        ]),
      ),
      ...(['routes.query@1', 'plugins.query@1'].includes(capability)
        ? {
            watch: {
              kind: 'subscription' as const,
              requestSchema: empty,
              snapshotSchema: boundedJson,
              eventSchema: boundedJson,
              requiredPermissions: [capability.split('@')[0]],
              open(
                value: unknown,
                _payload: JsonValue,
                ...
                return (value as PlatformService).watch(context, emit);
              },
            },
          }
        : {}),
...
export function createPlatformPlugin() {
  ...
  const bound = () => {
    if (!environment) throw Error('PLATFORM_NOT_BOUND');
    return environment;
  };
  function routes(): readonly PublicRoute[] {
    return bound().model.routes.map((route) => ({
      routeId: route.contribution.id,
      ownerPluginId: route.ownerPluginId,
      ancestry: route.ancestry,
      paramsSchema: bound().model.paramsSchema(route.contribution.id),
      state: route.state,
    }));
  }
  function current(): PublicRouteLocation | null {
    const env = bound(),
      resolved = env.model.resolve(env.location());
    if (resolved.state !== 'MATCHED') return null;
    return {
      routeId: resolved.routeId,
      ownerPluginId: env.model.routes.find((r) => r.contribution.id === resolved.routeId)!
        .ownerPluginId,
      ancestry: resolved.chain,
      params: resolved.context.params,
    };
  }
  function navigation(items = bound().model.navigation): readonly PublicNavigation[] {
    ...
    return items.map((item: ResolvedNavigation) => {
      ...
      const location =
        destination.state === 'LINK' ? env.model.resolve(destination.href) : undefined;
      return {
        ...
        ...(destination.state === 'DISABLED' ? { diagnostic: destination.diagnostic.code } : {}),
        children: navigation(item.children),
      };
    });
  }
  function plugins(): readonly PluginSummary[] {
    const env = bound(),
      ...
      catalog = env.catalog ?? [];
    const ids = new Set([
      ...env.runtime.candidates.map((p) => p.descriptor.id),
      ...records.map((p) => p.manifest.id),
      ...catalog.map((p) => p.record.manifest.id),
    ]);
    return [...ids].sort().map((id) => {
      const candidate = env.runtime.candidates.find((p) => p.descriptor.id === id),
        record = records.find((p) => p.manifest.id === id);
      const core = env.runtime.resolution.coreClosure.has(id),
        kind = candidate?.kind ?? 'restricted';
      return {
        id,
        label: catalog.find((p) => p.record.manifest.id === id)?.label ?? id,
        version: candidate?.descriptor.version ?? null,
        state: env.runtime.plugins.get(id)?.state ?? 'MISSING',
        kind,
        core,
        manageable: kind === 'restricted' && !core,
        installedVersions: env.store.listVersions(id).map((p) => p.manifest.version),
        versions: [
          ...new Set([
            ...catalog
              .filter((p) => p.record.manifest.id === id)
              .map((p) => p.record.manifest.version),
            ...env.store.listVersions(id).map((p) => p.manifest.version),
          ]),
        ].sort(),
        configuration: record
          ? { version: record.manifest.version, enabled: record.config.enabled }
          : null,
      };
    });
  }
  const snapshot = (capability: PlatformCapabilityId) =>
    capability === 'routes.query@1'
      ? json({ routes: routes(), current: current(), navigation: navigation() })
      : json(plugins());
  function notify(capability: PlatformCapabilityId) {
    for (const emit of watchers.get(capability) ?? []) {
      try {
        emit(snapshot(capability));
      } catch {
        /* A failed observer cannot change platform state. */
      }
    }
  }
  function manage(action: string, payload: Record<string, JsonValue>) {
    ...
    if (
      env.runtime.resolution.coreClosure.has(id) ||
      env.runtime.candidates.some((p) => p.descriptor.id === id && p.kind === 'builtin')
    )
      throw Error('PLUGIN_READ_ONLY');
    if (action === 'install') {
      const entry = env.catalog?.find(
        (p) => p.record.manifest.id === id && p.record.manifest.version === payload.version,
      );
        ...
        current
          ? {
              ...
              config: {
                ...
                grantedPermissions: current.config.grantedPermissions.filter((permission) =>
                  entry.record.manifest.permissions.includes(permission),
                ),
              },
            }
          : entry.record,
  ...
  const plugin: PluginDefinition = {
    ...
    roles: ['provider'],
    requires: [],
    provides: PLATFORM_CAPABILITIES,
    activate({ capabilities }) {
      for (const capability of PLATFORM_CAPABILITIES)
        capabilities.register<PlatformService>(capability, {
          invoke(action, payload, context) {
            ...
            if (capability === 'routes.query@1')
              return json(
                action === 'list' ? routes() : action === 'current' ? current() : navigation(),
              );
            if (capability === 'routes.navigate@1') {
              ...
              env.recordAudit({
                ...
                ownerPluginId: env.model.routes.find((r) => r.contribution.id === input.routeId)!
                  .ownerPluginId,
                timestamp: Date.now(),
              });
            ...
            if (capability === 'plugins.manage@1')
              return manage(action, payload as Record<string, JsonValue>);
            if (capability === 'audit.query@1') return json(env.audit());
            const diagnostics =
              env.diagnostics?.() ??
              inspect(env.runtime, {
                listHostContributions: () => env.model.listHostContributions(env.location()),
              });
            return capability === 'diagnostics.export@1'
              ? {
          ...
          watch(context, emit) {
            ...
            const listeners = watchers.get(capability) ?? new Set();
            watchers.set(capability, listeners);
            listeners.add(emit);
            return {
  ...
  return {
    plugin,
    bind(value: PlatformEnvironment) {
      if (environment) throw Error('PLATFORM_ALREADY_BOUND');
      environment = { ...value, catalog: frozenCopy(value.catalog ?? []) };
    },
    notify,
  };
}
