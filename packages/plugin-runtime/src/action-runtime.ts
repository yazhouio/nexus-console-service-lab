...
export interface ActionDriver {
  connect(input: ActionInput): Promise<ActionSession>;
}
...
export function createActionRuntime(options: {
  ...
  driver: ActionDriver;
  timeoutMs?: number;
  canUseCapability?: (owner: string, capability: `${string}@${number}`) => boolean;
}) {
  const points = new Map(
    frozenCopy(options.registry.listExtensionPoints()).map((p) => [
      uiKey(p.ownerPluginId, p.contribution.id),
      p,
    ]),
  );
  const definitions = new Set(
    options.registry.listActions().map((a) => uiKey(a.ownerPluginId, a.contribution.id)),
  );
  const relations = frozenCopy(
    options.registry
      .listExtensions()
      .filter(
        (e): e is OwnedContribution<ActionContributionDefinition> =>
          e.contribution.kind === 'action',
      )
      .map((entry) => {
        const definition = entry.contribution;
        const point = points.get(uiKey(definition.point.ownerPluginId, definition.point.id));
        const admission = point
          ? admitContribution(entry.ownerPluginId, definition, point.contribution, options.policy)
          : { authorized: false, reason: 'POINT_MISSING' };
        return {
          ...entry,
          ...admission,
          reason:
            admission.reason ??
            (point ? conditionError(point.contribution, definition) : undefined) ??
            (!definitions.has(uiKey(entry.ownerPluginId, definition.actionId))
              ? 'ACTION_MISSING'
              : undefined),
        };
      }),
  ...
  const outcomes: {
    readonly invocationId: string;
    readonly state: ActionOutcome['state'];
  ...
  function resolvePoint(owner: string, ref: ExtensionPointRef, context: JsonValue) {
    ...
    if (
      !point ||
      point.contribution.contractMajor !== ref.contractMajor ||
      point.contribution.kind !== 'action'
    )
      throw new UiError('POINT_CONTRACT_MISMATCH');
    assertUiJson(context);
    if (!matchesContext(point.contribution.contextSchema, context))
      throw new UiError('CONTEXT_INVALID');
  ...
  const api = {
    query(owner: string, point: ExtensionPointRef, context: JsonValue) {
      ...
      return relations
        .filter(
          (r) =>
            r.contribution.point.ownerPluginId === point.ownerPluginId &&
            r.contribution.point.id === point.id,
        )
        .sort((a, b) => comparePointContributions(contract, a, b))
        .map((r) => ({
          ...r,
          ...actionAvailability(
            contract,
            r.contribution,
            context,
            (id) => options.canUseCapability?.(r.ownerPluginId, id) ?? false,
          ),
        }));
    },
    invoke(
      owner: string,
      pointRef: ExtensionPointRef,
      ref: ContributionRef,
      context: JsonValue,
      input: { payload?: JsonValue; timeoutMs?: number; signal?: AbortSignal } = {},
    ) {
      const point = resolvePoint(owner, pointRef, context);
      const relation = api
        .query(owner, pointRef, context)
        .find((r) => r.ownerPluginId === ref.ownerPluginId && r.contribution.id === ref.id);
      if (!relation || relation.reason)
        throw new UiError(relation?.reason ?? 'ACTION_NOT_AVAILABLE');
      if (!relation.visible || relation.disabled) throw new UiError('ACTION_DISABLED');
      const payload = input.payload ?? null;
      assertUiJson(payload);
      if (point.payloadSchema && !matchesContext(point.payloadSchema, payload))
        throw new UiError('PAYLOAD_INVALID');
      const timeoutMs = input.timeoutMs ?? options.timeoutMs ?? 30_000;
      if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 300_000)
        throw new UiError('INVALID_ACTION_TIMEOUT');
      if (active.size >= 128) throw new UiError('ACTION_RESOURCE_LIMIT');
      const invocationId = crypto.randomUUID();
      const controller = new AbortController();
      let resolve!: (outcome: ActionOutcome) => void;
      const result = new Promise<ActionOutcome>((r) => {
        resolve = r;
      });
      let completed = false,
      ...
      const finish = (outcome: ActionOutcome) => {
        ...
        const snapshot = frozenCopy(outcome);
        outcomes.push({
          invocationId: snapshot.invocationId,
          state: snapshot.state,
          ...(snapshot.state === 'failed' ? { code: snapshot.code } : {}),
        });
        if (outcomes.length > 200) outcomes.shift();
        resolve(snapshot);
      };
      ...
      active.set(invocationId, {
        ownerPluginId: ref.ownerPluginId,
        actionId: relation.contribution.actionId,
        cancel,
      });
      input.signal?.addEventListener('abort', cancel, { once: true });
      const snapshot = frozenCopy(context),
        payloadSnapshot = frozenCopy(payload);
      if (input.signal?.aborted) cancel();
        ...
        track(
          Promise.resolve().then(async () => {
            if (completed) return;
            try {
              session = await options.driver.connect({
                invocationId,
                ownerPluginId: ref.ownerPluginId,
                actionId: relation.contribution.actionId,
                context: snapshot,
              ...
              void Promise.resolve()
                .then(() => (completed ? undefined : session!.invoke()))
                .then((value) => {
                  assertUiJson(value);
              ...
              finish({ invocationId, state: 'failed', code: 'ACTION_CONNECTION_FAILED' });
            }
          }),
        );
    ...
    inspect() {
      return frozenCopy({
        active: [...active].map(([invocationId, value]) => ({
          invocationId,
    ...
    async dispose() {
      closed = true;
      for (const invocation of active.values()) invocation.cancel();
      await api.settled();
    },
  };
  return api;
