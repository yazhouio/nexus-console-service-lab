...
export interface SurfaceDefinitionRegistry {
  get(pluginId: PluginId, surfaceId: string): SurfaceDefinitionRecord | undefined;
  list(pluginId?: PluginId): readonly SurfaceDefinitionRecord[];
}

export function projectSurfaceDefinitions(
  registry: ContributionRegistry,
  versionOf: (owner: PluginId) => string | undefined,
  ...
  const records: readonly SurfaceDefinitionRecord[] = Object.freeze(
    registry.listUiSurfaces().flatMap(({ ownerPluginId, contribution }) => {
      ...
      return [
        Object.freeze({
          ...
          definition: Object.freeze({ id: contribution.id }),
        }),
      ];
    }),
  );
  const byId = new Map(
    records.map((record) => [uiKey(record.pluginId, record.definition.id), record]),
  );
  return Object.freeze({
    list: (owner?: PluginId) =>
      owner === undefined
        ? records
        : Object.freeze(records.filter((record) => record.pluginId === owner)),
