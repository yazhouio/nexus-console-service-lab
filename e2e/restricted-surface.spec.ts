...
async function mount(page: Page) {
  ...
  await expect(page.getByTestId('restricted-surface')).toHaveAttribute(
    'data-bridge-state',
    'CONNECTED',
  );
}

test('loads on demand, calls Unary Bridge, documents cooperative isolation, and cleans up', async ({
  page,
}) => {
  ...
  const props = await page.evaluate(() => {
    const child = document.querySelector('iframe')?.contentWindow as
      | (Window & {
  ...
  expect(Object.keys((props?.bridge ?? {}) as object).sort()).toEqual([
    'nonce',
    'protocolVersion',
    'surfaceInstanceId',
  ]);
...
test('artifact failure affects the Surface only', async ({ page }) => {
  await ready(page);
  await page.route('**/plugins/kubeeye/1.0.0/**', (route) =>
...
test('mounts Route and Extension independently, isolates failure, and unmounts in reverse order', async ({
  page,
}) => {
  await ready(page);
  await mount(page);
  ...
  const props = await page.evaluate(() =>
    Array.from(document.querySelectorAll('iframe')).map((iframe) => {
      const child = iframe.contentWindow as Window & {
        $wujie?: {
          props: {
            bridge: { surfaceInstanceId: string };
            surface: { initialParameters: unknown; layout: unknown };
          };
        };
      };
      return child.$wujie?.props;
  ...
  expect(props.map((props) => props?.surface.initialParameters)).toEqual([
    { view: 'route' },
    undefined,
  ]);
  ...
  await page.evaluate(() =>
    document
      ...
      ?.contentWindow?.dispatchEvent(new ErrorEvent('error', { message: 'route failed' })),
  );
  await expect(page.getByTestId('surface-state')).toHaveText('ERROR');
  await expect(page.getByTestId('extension-state-kubeeye')).toHaveText('MOUNTED');
  await expect(page.getByTestId('plugin-state-kubeeye')).toHaveText('ACTIVE');
...
test('streams Host cluster changes to independent sessions and exposes safe Runtime facts', async ({
  page,
}) => {
  await ready(page);
  ...
  const snapshot = async () =>
    JSON.parse((await page.getByTestId('runtime-snapshot').textContent()) ?? '{}');
  await expect
    .poll(async () =>
      (await snapshot()).plugins
        .find((plugin: { id: string }) => plugin.id === 'kubeeye')
        .surfaces[0].instances.map(
          (instance: { bridgeSession?: { subscriptionCount: number } }) =>
            instance.bridgeSession?.subscriptionCount ?? 0,
        ),
    )
    .toEqual([1, 1]);
  ...
  await page.evaluate(() =>
    document
      .querySelector('iframe')
      ?.contentWindow?.dispatchEvent(new ErrorEvent('error', { message: 'Bearer secret-token' })),
  );
  await expect(page.getByTestId('surface-state')).toHaveText('ERROR');
  await expect
    .poll(
      async () =>
        (await snapshot()).plugins.find((plugin: { id: string }) => plugin.id === 'kubeeye')
          .surfaces[0].instances.length,
    )
    .toBe(1);
  await expect
    .poll(
      async () =>
        (await snapshot()).ui.scopes.find(
          (scope: { owner: string; kind: string }) =>
            scope.owner === 'kubeeye' && scope.kind === 'root',
        )?.execution.phase,
    )
    .toBe('failed');
  await expect(page.locator('iframe')).toHaveCount(1);
...
test('Bridge grants do not authorize same-origin network access; the Backend enforces its own permissions', async ({
  page,
  context,
  baseURL,
}) => {
  ...
  expect(network).toEqual({
    allowed: 200,
    denied: 403,
    body: { code: 'BACKEND_PERMISSION_DENIED' },
  });
  await expect(page.getByTestId('surface-state')).toHaveText('MOUNTED');
});
