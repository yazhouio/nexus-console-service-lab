...
test('Host mounts the independently declared A → B → C chain and updates same-key Context', async ({ page }) => {
  await page.goto('/__fixtures__/ui-composition');
...
test('hidden retains execution, unmount rebuilds, and A → B → A creates a fresh Scope A₂', async ({ page }) => {
  await visit(page);
...
test('invalid Context is independent from execution; a rejected new key removes the old object', async ({ page }) => {
  await visit(page);
  const original = await page.getByTestId('b-instance').innerText();
...
test('occurrences are independent and filtering rebuilds only the removed contribution', async ({ page }) => {
  await visit(page);
...
test('a failed B Attempt preserves its Overlay in the same Scope and retry recovers it', async ({ page }) => {
  await visit(page);
  ...
  await expect(page.getByTestId('c-context')).toContainText('"value":2');
  const after = (await inspect(page)).scopes.find((s: any) => s.owner === 'ui-b' && s.kind === 'contribution');
...
test('parent Attempt failure ends child scopes but preserves its own Overlay; ending Scope clears everything', async ({ page }) => {
  await visit(page);
  ...
  const after = (await inspect(page)).scopes.find((s: any) => s.owner === 'ui-a' && s.kind === 'root');
  expect(after.id).toBe(before.id);
  expect(after.attemptId).not.toBe(before.attemptId);
...
test('real containers coordinate content-sized and bounded dimensions without changing Context revision', async ({ page }) => {
  await visit(page);
...
test('Builtin render failure uses the same retry core and independent observation dimensions', async ({ page }) => {
  await visit(page);
  const b = await page.getByTestId('b-instance').innerText();
  ...
  expect(observation.contributions.find((c: any) => c.ref.ownerPluginId === 'ui-b').execution.phase).toBe('ready');
  expect(observation.contributions.find((c: any) => c.ref.ownerPluginId === 'ui-local').execution.phase).toBe('failed');
  await button(page, 'Retry local-card').click();
  await expect(page.getByTestId('ui-local')).toBeVisible();
  await expect(page.getByTestId('b-instance')).toHaveText(b);
});

test('Overlay presentation failure remains pending, retries independently, and Escape cancels once', async ({ page }) => {
  await visit(page);
  await page.route('**/plugins/ui-b/1.0.0/', route => route.fulfill({ status: 503, body: 'Unavailable' }));
  await button(page, 'Open B overlay').click();
...
test('removing a physical Anchor invalidates its subtree and a late artifact cannot revive it', async ({ page }) => {
  let requested = false;
  let release!: () => void;
  ...
  await page.getByTestId('ui-a').locator('[data-nexus-slot-anchor]').first().evaluate(el => el.remove());
  await expect.poll(async () => (await inspect(page)).scopes.length).toBe(1);
  release();
  await expect.poll(async () => JSON.parse(await page.getByTestId('ui-instances').innerText()).length).toBe(1);
  await expect(page.getByTestId('ui-b')).toHaveCount(0);
  await page.unroute('**/plugins/ui-b/1.0.0/');
  await button(page, 'Toggle Slot').click();
  await button(page, 'Toggle Slot').click();
