...
test('preserves parent Layout state while changing siblings and remounting only the leaf context', async ({
  page,
}) => {
  await page.goto('/');
      ...
      () =>
        (
          document.querySelector('iframe')!.contentWindow as Window & {
            $wujie: { props: { routeContext: unknown } };
          }
        ).$wujie.props.routeContext,
    );
  expect(await context()).toEqual({
    routeId: 'node-alert-messages',
    pathname: '/clusters/demo/nodes/n1/alert-messages',
    params: { cluster: 'demo', node: 'n1' },
    search: '',
  });
  await page
    .getByRole('navigation', { name: 'Node tabs' })
...
test('shows a retryable Surface error without redirecting or failing plugin bootstrap', async ({
  page,
}) => {
...
test('conflicting child diagnostics preserve the parent, siblings, and ACTIVE inspection axis', async ({
  page,
}) => {
  await page.goto('/');
  await page.getByRole('button', { name: 'Install KubeEye 2.0.0' }).click();
  ...
  await page.evaluate(() => {
    ...
    const record = snapshot.records.find(
      (r: { manifest: { version: string } }) => r.manifest.version === '2.0.0',
    );
    record.manifest.contributions.routes.push({
      id: 'conflicting-alerts',
      parentRouteId: 'node-detail',
      point: { ownerPluginId: 'cluster', id: 'node.children', contractMajor: 1 },
      path: 'alert-messages',
      surfaceId: 'overview',
    });
  ...
  expect(
    snapshot.contributions.routes.find((r: { id: string }) => r.id === 'node-alert-messages').host,
  ).toMatchObject({ state: 'QUARANTINED', diagnostics: [{ code: 'ROUTE_CONFLICT' }] });
});

test('startup failure is distinct from an unmatched URL', async ({ page }) => {
  await page.addInitScript(() =>
    localStorage.setItem('nexus.plugin-installations.v1', 'invalid-json'),
  );
