...
test('concurrent Surfaces, hidden UI and Overlay share a link until their final DOM is removed', async ({ page }) => {
...
test('Builtin in a Restricted anchor loads the same URL separately in the existing physical ShadowRoot', async ({ page }) => {
  await page.goto('/__fixtures__/styling');
  await expect(page.getByTestId('ui-local')).toHaveCount(2);
  ...
  expect(await restricted.locator(localCss).getAttribute('href')).toBe(await page.locator(`head ${localCss}`).getAttribute('href'));
...
test('Host theme attribute updates CSS Modules in Document, ShadowRoot and Overlay without remounting', async ({ page }) => {
  await page.goto('/__fixtures__/styling');
  await expect(page.getByTestId('ui-local')).toHaveCount(2);
  await page.getByRole('button', { name: 'Toggle Restricted parent' }).click();
  await expect(page.getByTestId('style-restricted-container').getByTestId('ui-local')).toBeVisible();
  await page.locator(first).getByRole('button', { name: 'Open Local overlay' }).click();
  await expect(page.getByRole('dialog').getByTestId('ui-local')).toBeVisible();
  const scope = await page.locator(first).getAttribute('data-execution-scope');
  await page.addStyleTag({ content: ':root[data-nexus-theme="acceptance"] { --nexus-color-text-primary: rgb(80, 30, 110); --nexus-color-surface: rgb(237, 232, 248); --nexus-space-4: 21px; }' });
  await page.evaluate(() => { document.documentElement.dataset.nexusTheme = 'acceptance'; });
...
test('CSS error is an artifact failure, renders no business DOM and explicitly retries a new load', async ({ page }) => {
  let fail = true, requests = 0;
  await page.route('**/static/plugin-css/ui-local.*.css', route => { requests++; return fail ? route.abort() : route.continue(); });
  await page.goto('/__fixtures__/styling');
...
test('CSS waiting does not consume the UI-ready timeout and cancellation preserves the other waiter', async ({ page }) => {
  let resume!: () => void;
  const gate = new Promise<void>(resolve => { resume = resolve; });
  await page.route('**/static/plugin-css/ui-local.*.css', async route => { await gate; await route.continue(); });
...
test('CSS deadline reports artifact failure and Core CSS failure keeps independent recovery usable', async ({ page }) => {
...
test('compiled production HTML owns only platform CSS; route departure releases business assets', async ({ page, request }) => {
...
test('Restricted CSS 404 retries the same URL, inherits Host tokens and is destroyed with its Wujie instance', async ({ page }) => {
  let fail = true, requests = 0;
  await page.route('**/plugins/kubeeye/1.0.0/main.css', async route => {
    ...
    await route.fulfill({ response, body: (await response.text()) + ':root{--kubeeye-private-probe:7px}@font-face{font-family:kubeeye-probe;src:local("Arial")}' });
  });
  ...
  expect(requests).toBe(2);
  await expect.poll(() => page.getByTestId('restricted-surface').evaluate(element => getComputedStyle(element).getPropertyValue('--kubeeye-private-probe').trim())).toBe('7px');
  const fonts = () => page.evaluate(() => [...document.head.querySelectorAll('style')].filter(style => style.textContent?.includes('kubeeye-probe')).length);
  ...
  await page.addStyleTag({ content: ':root[data-nexus-theme="restricted-test"] { --nexus-color-text-primary: rgb(80, 30, 110); --nexus-color-surface: rgb(237, 232, 248); }' });
  await page.evaluate(() => { document.documentElement.dataset.nexusTheme = 'restricted-test'; });
  await expect(page.getByTestId('restricted-surface')).toHaveCSS('color', 'rgb(80, 30, 110)');
  await expect(page.getByTestId('restricted-surface')).toHaveCSS('background-color', 'rgb(237, 232, 248)');
...
test('CSS completion rejects a container moved into an unmanaged physical root despite patched DOM accessors', async ({ page }) => {
  let resume!: () => void;
  const gate = new Promise<void>(resolve => { resume = resolve; });
  await page.route('**/static/plugin-css/ui-local.*.css', async route => { await gate; await route.continue(); });
  await page.goto('/__fixtures__/styling');
