...
test('Builtin layouts execute in managed scopes and retain their scope and state across child routes', async ({
  page,
}) => {
  ...
  await page.evaluate(() => {
    history.pushState(null, '', '/clusters/demo/nodes/n1');
...
test('a Console root render failure enters independent recovery without reversing Ready', async ({
  page,
}) => {
  await page.goto('/');
  await expect(page.locator('[data-managed-presentation="root-presentation"]')).toHaveAttribute(
...
test('independent recovery restores the previous Restricted configuration after an uninstall', async ({
  page,
}) => {
  await page.goto('/');
  ...
  await expect(page.getByRole('heading', { name: 'Console presentation failed' })).toBeVisible();
  await expect(
    page.getByRole('button', { name: 'Disable kubeeye and reload', exact: true }),
  ).toHaveCount(0);
