...
const variants = [
  'baseline',
  'direct-generated-list',
  'versioned-url-no-hash',
  'live-token-copy',
  'no-platform-tokens',
  'plugin-token-defaults',
  'global-selectors',
  'document-only',
  'url-only-dedupe',
  'no-dedupe',
  'no-reference-lifetime',
  'no-final-release',
  'no-ready-gate',
  'no-failure-rollback',
  'retain-failed-cache',
  'release-on-abort',
  'handwritten-stale-assets',
  'no-deadline',
  'host-static-import',
];
const cases = [
  ...
  'bounded-failure',
  'slot-overlay-lifetime',
  'single-css-owner',
];
const sources = [
  'packages/plugin-runtime/src/ui/runtime.ts',
  'packages/plugin-runtime/src/contribution.ts',
  'packages/plugin-runtime/src/browser/ui-host.ts',
  'packages/plugin-runtime/src/browser/wujie-driver.ts',
  'packages/plugin-runtime/src/browser/wujie-plugin-adapter.ts',
  'packages/plugin-runtime/node_modules/wujie/lib/index.js',
  'docs/validation/plugin-styling-ablation.mjs',
  'docs/validation/plugin-styling-ablation.browser.js',
];
const builds = {};
...
try {
  for (const { revision, version, filenameHash } of [
    ...
    await writeFile(
      join(work, 'business.module.css'),
      `.cardV${version} { color: rgb(${version === 1 ? '13, 57, 91' : '91, 37, 13'}); padding: ${version * 7}px; }`,
    );
    const rsbuild = await createRsbuild({
      ...
      rsbuildConfig: {
        ...
        output: {
          distPath: { root: join(work, 'dist') },
          assetPrefix: `/build/${revision}/`,
          manifest: true,
          injectStyles: false,
          filenameHash,
        },
    ...
    if (!entry?.initial?.css?.length || !entry.initial.js?.length)
      throw Error('Build did not emit CSS/JS entry assets');
    builds[revision] = {
      directory: join(work, 'dist'),
      css: entry.initial.css,
      js: entry.initial.js,
  ...
  const wujie = await readFile(
    join(root, 'packages/plugin-runtime/node_modules/wujie/lib/index.js'),
  );
  ...
  server = createServer(async (req, res) => {
    ...
    try {
      if (url.pathname === '/')
        return send(
          `<!doctype html><html><head><script src="/wujie.js"></script>${builds.v1.js.map((src) => `<script src="${src}"></script>`).join('')}<script src="/harness.js"></script></head><body></body></html>`,
        );
      if (url.pathname === '/wujie.js') return send(wujie, 'text/javascript');
      if (url.pathname === '/harness.js') return send(harness, 'text/javascript');
      if (url.pathname === '/plugin.html')
        return send(
          `<!doctype html><html><head><link rel="stylesheet" href="/restricted.css${url.search}"></head><body><div class="restrictedOwned hostPrivate">Restricted</div><div id="anchor"></div><script>window.__fixtureReady = true;</script></body></html>`,
        );
      if (url.pathname === '/retry-plugin.html')
        return send(
          `<!doctype html><html><head><link rel="stylesheet" href="/flaky.css${url.search}"></head><body><div class="owned">Recovered</div><script>window.__fixtureReady = true;</script></body></html>`,
        );
      if (url.pathname === '/restricted.css')
        return send(
          '.restrictedOwned{color:var(--nexus-test-color, rgb(0,0,0));padding:19px}' +
            (url.searchParams.get('defaults') === 'true'
              ? ':root{--nexus-test-color:rgb(200, 0, 0)}'
              : ''),
          'text/css',
        );
      if (url.pathname === '/slow.css')
        return void setTimeout(() => send(productCss, 'text/css'), 180);
      if (url.pathname === '/hang.css') return; // Client timeout owns completion; server closes remaining sockets.
      if (url.pathname === '/flaky.css')
        return send(productCss, 'text/css', stats.requests.get(key) === 1 ? 404 : 200);
      ...
      send(String(error), 'text/plain', 500);
    }
  });
  ...
  for (const variant of variants) {
    ...
    for (const name of cases) {
      ...
      try {
        ...
        checks.push({ name, passed: false, infrastructureError: String(error), pageErrors });
      } finally {
        await page.close();
      }
    }
    ...
    console.log(
      JSON.stringify({
        ...
        failedChecks: checks
          .filter((c) => !c.passed)
          .map((c) => ({ name: c.name, error: c.error ?? c.infrastructureError })),
      }),
    );
    if (variant === 'baseline' && result.failed)
      throw Error('Baseline failed; fix the probe before interpreting mutations.');
  }
  const page = await browser.newPage();
  ...
  const evidence = {
    ...
    wujie: '2.1.0',
    scope:
      'Experimental browser asset loader over unchanged production UI core, real Wujie and three real Rsbuild CSS Modules builds (two source revisions, one no-hash output replacement). No production Browser UI host, React presentation, bridge handshake, Tailwind or StyleX integration certification. Single-factor variants unless explicitly identified as equivalent replacements/additions.',
    sourceSha256: Object.fromEntries(
      await Promise.all(
        sources.map(async (path) => [path, digest(await readFile(join(root, path)))]),
      ),
    ),
    builds: Object.fromEntries(
      Object.entries(builds).map(([key, { directory: _directory, ...value }]) => [key, value]),
    ),
    results,
    characterization: { wujieRetry },
  };
  await writeFile(
    join(root, 'docs/validation/plugin-styling-ablation-results.json'),
    JSON.stringify(evidence, null, 2) + '\n',
  );
  if (results.some((r) => r.checks.some((c) => c.infrastructureError)))
    throw Error('Infrastructure errors occurred; see results');
} finally {
  await browser?.close();
