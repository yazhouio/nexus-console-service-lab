...
(() => {
  ...
  function loader(variant) {
    ...
    function acquireOne(href, requestedRoot) {
      ...
      const cache = byRoot.get(cacheRoot) ?? new Map();
      byRoot.set(cacheRoot, cache);
      ...
      return {
        ...
        release() {
          ...
          if (variant === 'retain-failed-cache' && current.state === 'failed') {
            current.link.remove();
            return;
          }
    ...
    return {
      async acquire(asset, root, signal) {
        ...
        const cancelled = new Promise((_, reject) => {
          abort = () => reject(issue('CSS_CANCELLED'));
        });
  ...
  function fixture(variant, files, options = {}) {
    ...
    const resources = loader(variant),
    ...
    for (const owner of ['p', 'q']) {
      ...
      if (owner === 'p')
        transaction.context.registerExtensionPoint({
          id: 'children',
          kind: 'surface',
          contractMajor: 1,
        ...
        transaction.context.registerExtension({
          id: 'child',
          kind: 'surface',
          surfaceId: 'main',
    ...
    const driver = {
      allocate(anchor) {
        const node = element('placement', anchor);
        return { placement: node, dispose: () => node.remove() };
      },
      ...
      overlay() {
        ...
        const node = element('overlay-placement', dialog);
        dialog.showModal();
        return {
          placement: node,
          dispose() {
      ...
      async mount(input) {
        ...
        let node, lease;
        const render = () => {
          node = element(options.className ?? 'owned', container);
        };
        ...
        try {
          ...
          if (input.signal.aborted) {
            lease.release();
            throw new UiError('CSS_CANCELLED');
          }
          if (!node) render();
        } catch (error) {
          node?.remove();
          if (!input.signal.aborted)
            core.failAttempt(input.attemptId, error.code ?? 'CSS_LOAD_FAILED', 'artifact');
          throw error;
        }
        if (variant === 'release-on-abort')
          input.signal.addEventListener('abort', lease.release, { once: true });
        return {
          async dispose() {
            try {
              await options.beforeDomRemove?.(node);
              node.remove();
            } finally {
              lease.release();
            }
  ...
  async function sandbox(variant = 'baseline', url) {
    const container = element('restricted-container'),
      name = `probe-${crypto.randomUUID()}`;
    const errors = [];
    await window.wujie.startApp({
      name,
      el: container,
      url: url ?? uniqueURL('/plugin.html') + `&defaults=${variant === 'plugin-token-defaults'}`,
      alive: true,
      sync: false,
      fiber: false,
      degrade: false,
      loadError: (url, error) => errors.push({ url, message: error.message }),
    });
    ...
    if (variant === 'live-token-copy') {
      const copy = () =>
        root.host.style.setProperty(
          '--nexus-test-color',
          getComputedStyle(document.documentElement).getPropertyValue('--nexus-test-color'),
        );
      copy();
      observer = new MutationObserver(copy);
    ...
    return {
      ...
      async dispose() {
        observer?.disconnect();
        await window.wujie.destroyApp(name);
        container.remove();
      },
    };
  }
  function platform(variant) {
    if (variant !== 'no-platform-tokens')
      css(
        ':root{--nexus-test-color:rgb(20, 40, 60)}:root[data-theme=dark]{--nexus-test-color:rgb(180, 200, 220)}',
      );
  ...
  const tests = {
    async 'theme-inheritance'(variant) {
      const f = fixture(variant, [uniqueURL('/product.css')]),
        b = await sandbox(variant);
      const builtin = mount(f),
        dialog = document.createElement('dialog');
      document.body.append(dialog);
      const overlay = mount(f, element('overlay-presentation', dialog));
      dialog.showModal();
      try {
        await Promise.all([ready(builtin.root), ready(overlay.root)]);
        const values = () => [
          color(builtin.container.firstChild),
          color(b.root.querySelector('.restrictedOwned')),
          color(overlay.container.firstChild),
        ];
        const light = values();
        check(
          light.every((v) => v === 'rgb(20, 40, 60)'),
          'Initial semantics differ',
          light,
        );
        ...
        const dark = values();
        check(
          dark.every((v) => v === 'rgb(180, 200, 220)'),
          'Live theme did not reach all presentations',
          dark,
        );
    ...
    async 'selector-ownership'(variant) {
      const host = element('hostPrivate'),
        parent = element('pluginA'),
        nested = element('pluginB', parent);
      const buttonA = document.createElement('button'),
        buttonB = document.createElement('button');
      buttonA.className = 'pluginA-button';
      buttonB.className = 'pluginB-button';
      parent.prepend(buttonA);
      nested.append(buttonB);
      css('.hostPrivate{color:rgb(1,2,3)}.pluginB-button{color:rgb(3,2,1)}');
      const b = await sandbox(variant);
      try {
        css(
          variant === 'global-selectors'
        ...
        const observed = {
          host: color(host),
          own: color(buttonA),
          nested: color(buttonB),
          restricted: color(b.root.querySelector('.hostPrivate')),
        };
        check(
          observed.host === 'rgb(1, 2, 3)' && observed.nested === 'rgb(3, 2, 1)',
          'Another owner was restyled',
          observed,
        );
        check(observed.own === 'rgb(255, 0, 0)', 'Own style absent', observed);
        check(
          observed.restricted !== observed.host,
          'Host private class crossed Wujie root',
          observed,
        );
        return observed;
      } finally {
    ...
    async 'shared-lifetime'(variant) {
      ...
      try {
        const a = mount(f),
          b = mount(f);
        await Promise.all([ready(a.root), ready(b.root)]);
        check(
          links(document).length === 1,
          'Concurrent presentations duplicated stylesheet',
          links(document).length,
        );
        a.root.dispose();
        await f.core.settled();
        check(
          getComputedStyle(b.container.firstChild).padding === '17px',
          'First release removed surviving presentation CSS',
          getComputedStyle(b.container.firstChild).padding,
        );
        b.root.dispose();
        await f.core.settled();
        check(
          links(document).length === 0,
          'Last release retained stylesheet',
          links(document).length,
        );
    ...
    async 'actual-style-root'(variant) {
      const f = fixture(variant, [uniqueURL('/product.css')]),
        b = await sandbox(variant),
        c = await sandbox(variant);
      try {
        const a = mount(f);
        await ready(a.root);
        const insideB = mount(f, b.root.querySelector('#anchor')),
          insideC = mount(f, c.root.querySelector('#anchor'));
        ...
        const padding = [a, insideB, insideC].map(
          (item) => getComputedStyle(item.container.firstChild).padding,
        );
        check(
          count.every((n) => n === 1),
          'Same URL must mount once in each existing CSS root',
          { count, padding },
        );
        check(
          padding.every((v) => v === '17px'),
          'Builtin CSS did not reach its physical presentation',
          padding,
        );
        await f.core.dispose();
        check(
          roots.every((root) => !links(root).length),
          'Existing roots retained CSS after disposal',
        );
    ...
    async 'ready-gate'(variant) {
      ...
      try {
        ...
        await pause(30);
        check(
          !a.container.querySelector('.owned'),
          'Business DOM appeared before required CSS',
          a.root.execution,
        );
        await ready(a.root);
    ...
    async 'failure-rollback-retry'(variant) {
      ...
      try {
        const a = mount(f);
        await until(() => a.root.execution.phase === 'failed', 'CSS 404 did not fail UI');
        await f.core.settled();
        const failed = a.root.execution;
        check(
          failed.stage === 'artifact' && !a.container.firstChild,
          'Failure attribution or presentation rollback missing',
          failed,
        );
        check(
          links(document).length === 0,
          'Partial load leaked references',
          links(document).length,
        );
        a.root.retry();
        await ready(a.root);
        ...
        return {
          failure: { code: failed.error, stage: failed.stage },
          retry: a.root.execution.phase,
          padding,
        };
      } finally {
        await f.dispose();
    ...
    async 'cancel-shared-load'(variant) {
      ...
      try {
        ...
        await f.core.settled();
        check(
          !a.container.firstChild && getComputedStyle(b.container.firstChild).padding === '17px',
          'Cancellation affected another consumer',
        );
        b.root.dispose();
        await f.core.settled();
        await pause(20);
        check(
          !links(document).length,
          'Cancelled/finished load retained resource',
          links(document).length,
        );
    ...
    async 'dom-cleanup-order'(variant) {
      ...
      try {
        ...
        check(
          observed.connected && observed.padding === '17px',
          'CSS released before asynchronous DOM cleanup',
          observed,
        );
        finish();
        await f.core.settled();
    ...
    async 'build-asset-identity'(variant, builds) {
      ...
      try {
        ...
        await ready(a.root);
        const observed = {
          className: window.buildClass,
          color: color(a.container.firstChild),
          padding: getComputedStyle(a.container.firstChild).padding,
        };
        check(
          observed.color === 'rgb(91, 37, 13)' && observed.padding === '14px',
          'Rebuilt JS was paired with stale/manual CSS assets',
          observed,
        );
        a.root.dispose();
        await f.core.settled();
    ...
    async 'bounded-failure'(variant) {
      ...
      try {
        const a = mount(f);
        await until(
          () => a.root.execution.phase === 'failed',
          'CSS without a deadline remained starting',
          800,
        );
        await f.core.settled();
        check(
          a.root.execution.error === 'CSS_LOAD_TIMEOUT' && a.root.execution.stage === 'artifact',
          'Timeout lost asset attribution',
          a.root.execution,
        );
    ...
    async 'slot-overlay-lifetime'(variant) {
      ...
      try {
        const a = mount(f);
        await ready(a.root);
        const anchor = element('slot-anchor', a.container.firstChild);
        const input = { id: 'children', contextKey: 'A', context: {} };
        const occurrence = f.core.mountSlot(a.root.attemptId, anchor, input);
        const child = () =>
          f.core.snapshot(a.root.attemptId).occurrences[0].contributions[0].execution;
        ...
        check(
          links(document).length === 1,
          'Shared Slot emitted duplicate stylesheets',
          links(document).length,
        );
        const handle = f.core.openOverlay(a.root.attemptId, 'main', null);
        await until(
          () =>
            f.core.overlaySnapshot(a.root.attemptId).overlays.find((o) => o.handle === handle)
              ?.execution?.phase === 'ready',
          'Overlay never became ready',
        );
        f.core.unmountSlot(a.root.attemptId, occurrence);
        await f.core.settled();
        const overlayNode = document.querySelector('dialog .owned');
        check(
          overlayNode && getComputedStyle(overlayNode).padding === '17px',
          'Child teardown removed owner/overlay CSS',
        );
        a.root.dispose();
        await f.core.settled();
        check(
          !links(document).length && !document.querySelector('dialog'),
          'Owner end leaked overlay or stylesheet',
        );
    ...
    async 'single-css-owner'(variant) {
      ...
      if (variant === 'host-static-import') {
        ...
        imported.href = url;
        await new Promise((resolve, reject) => {
          imported.onload = resolve;
          imported.onerror = reject;
      ...
      try {
        ...
        const remaining = [...document.querySelectorAll('link[rel=stylesheet]')].filter(
          (link) => link.href === new URL(url, location.href).href,
        ).length;
        const padding = getComputedStyle(sentinel).padding;
        check(
          remaining === 0 && padding === '0px',
          'Host static import retained CSS after Runtime release',
          { remaining, padding },
        );
  ...
  window.runStylingCase = async ({ variant, name, builds }) => {
    platform(variant);
    try {
      return { passed: true, observed: await tests[name](variant, builds) };
    } catch (error) {
  ...
  window.characterizeWujieRetry = async () => {
    ...
    const firstResult = {
      loadErrors: first.errors.length,
      padding: getComputedStyle(first.root.querySelector('.owned')).padding,
    };
    await first.dispose();
    const second = await sandbox('baseline', url);
    const secondResult = {
      loadErrors: second.errors.length,
      padding: getComputedStyle(second.root.querySelector('.owned')).padding,
    };
    ...
    return {
      ...
      retry: secondResult,
      recovered:
        firstResult.loadErrors > 0 &&
        secondResult.loadErrors === 0 &&
        secondResult.padding === '17px',
    };
  };
})();
