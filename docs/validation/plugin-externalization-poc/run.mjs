...
const capture = async (file) => {
  ...
  report.snapshots[file] = digest(bytes);
  return bytes;
};
...
try {
  ...
  await command(
    ...
    [
      ...
      'tsc',
      '-p',
      'packages/plugin-runtime/tsconfig.build.json',
      '--outDir',
      join(sdkDir, 'dist'),
    ],
    repo,
  ...
  for (const file of [
    'packages/plugin-runtime/src/ui-react.tsx',
    'packages/plugin-runtime/src/browser/ui-host.ts',
    'packages/plugin-runtime/src/browser/artifact-assets.ts',
  ...
  const pkg = JSON.parse(
    await readFile(join(repo, 'packages/plugin-runtime/package.json'), 'utf8'),
  );
  await writeFile(
    ...
    JSON.stringify(
      {
        ...
        exports: pkg.exports,
        files: ['dist'],
        dependencies: { wujie: '2.1.0', 'react-router': '8.3.1' },
  ...
  for (const file of [
    'packages/plugin-build/artifact-css-loader.cjs',
    'packages/plugin-build/check-plugin-css.mjs',
    'packages/plugin-build/reject-unmanaged-css.cjs',
    'packages/plugin-build/artifact-css-closure.mjs',
  ]) {
    ...
    await writeFile(
      join(
        remote,
        'tools',
        file.split('/').at(-1).replace('artifact-css-closure.mjs', 'artifact-css-closure.ts'),
      ),
      bytes,
    );
  }
    ...
    JSON.stringify(
      {
        ...
        type: 'module',
        overrides,
        dependencies: { ...common, '@module-federation/enhanced': '2.9.0' },
      },
      null,
      2,
    ),
  ...
  const installations = await Promise.allSettled(
    [host, remote].map((dir, index) =>
      command(
        `install-${index ? 'remote' : 'host'}`,
        'npm',
  ...
  for (const [label, dir] of [
    ...
    for (const name of [
      ...
      try {
        ...
        /* Not a direct node_modules entry. */
      }
    }
  }
  let slowCss = 0;
  async function serve(dir, label) {
    const server = createServer(async (request, response) => {
      ...
      let file = join(
        dir,
        pathname === '/' || (label === 'host' && !extname(pathname)) ? 'index.html' : pathname,
      );
      response.setHeader('Access-Control-Allow-Origin', '*');
      response.setHeader('Cache-Control', 'no-store');
      const event = { server: label, path: pathname, at: Date.now() };
      report.network.push(event);
      try {
        const content = await readFile(file);
        if (label === 'remote' && file.endsWith('.css') && slowCss)
          await new Promise((resolve) => setTimeout(resolve, slowCss));
        response.setHeader(
          'Content-Type',
          {
            '.html': 'text/html',
  ...
  const remoteOrigin = await serve(join(remote, 'dist'), 'remote'),
    hostOrigin = await serve(join(host, 'dist'), 'host');
  report.origins = { host: hostOrigin, remote: remoteOrigin };
  for (const variant of ['good', 'badreact', 'badsdk', 'loose', 'duplicatesdk', 'lazybad'])
    await command(
      `build-${variant}`,
      process.execPath,
      ['build.mjs', variant, remoteOrigin],
      remote,
    );
  ...
  assert.equal(
    report.network.filter((item) => item.server === 'remote' && item.path.endsWith('.css')).length,
    0,
    'no CSS preload before UI',
  );
  slowCss = 500;
  await normal.page.evaluate(async () => {
    await window.poc.start();
    window.poc.mount('a');
    window.poc.mount('b');
  });
  await normal.page.waitForFunction(
    () => document.querySelectorAll('link[data-nexus-artifact-css]').length === 2,
  );
  ...
  await normal.page.waitForFunction(
    () => window.poc.phase('a') === 'ready' && window.poc.phase('b') === 'ready',
  );
  slowCss = 0;
  ...
  assert.equal(
    await normal.page.locator('[data-probe] a').first().getAttribute('href'),
    '/from-host/linked',
  );
  ...
  report.cases.push({
    name: 'shared-identities-and-managed-css',
    passed: true,
    load,
    lazy,
    style,
  ...
  for (const [variant, extra, pattern] of [
    ['badreact', '', /does not satisfy|Unsatisfied/i],
    ['badsdk', '', /does not satisfy|Unsatisfied/i],
    ['good', '&missing=1', /shared|RUNTIME-006|not exist/i],
  ]) {
    const { page, logs } = await pageFor(variant, extra),
      result = await loaded(page);
    assert.equal(result.ok, false, `${variant} should reject: ${JSON.stringify(result)}`);
    assert.match(result.error, pattern);
    assert.equal(await styleCount(page), 0);
    await page.locator('#host-counter').click();
    assert.equal(await page.locator('#host-counter').textContent(), 'Host: 1');
    report.cases.push({ name: extra ? 'missing-shared-sdk' : variant, passed: true, result, logs });
    await page.close();
  }
  const loose = await pageFor('loose'),
    looseResult = await loaded(loose.page);
  assert.equal(looseResult.ok, true);
  assert.ok(
    loose.logs.some((item) => /does not satisfy|Unsatisfied/i.test(item.text)),
    'non-strict negative control must warn',
  );
  report.cases.push({
    name: 'non-strict-negative-control',
    passed: true,
    result: looseResult,
    logs: loose.logs,
  });
  await loose.page.close();
  const duplicate = await pageFor('duplicatesdk'),
    duplicateResult = await loaded(duplicate.page);
  ...
  await duplicate.page.evaluate(async () => {
    await window.poc.start();
    window.poc.mount('duplicate');
  });
  await duplicate.page.waitForFunction(() =>
    window.poc.errors.some((error) => error.includes('Host-bound UiProvider')),
  );
  report.cases.push({
    name: 'duplicate-sdk-negative-control',
    passed: true,
    result: duplicateResult,
    errors: await duplicate.page.evaluate(() => window.poc.errors),
    logs: duplicate.logs,
  });
  await duplicate.page.evaluate(() => window.poc.dispose());
  await duplicate.page.close();
  const lazyBad = await pageFor('lazybad'),
    lazyEntry = await loaded(lazyBad.page);
  assert.equal(lazyEntry.ok, true);
  const lazyFailure = await lazyBad.page.evaluate(() => window.poc.loadLazy());
  assert.equal(lazyFailure.ok, false);
  assert.match(lazyFailure.error, /react-dom/);
  assert.equal(await styleCount(lazyBad.page), 0);
  report.cases.push({
    name: 'lazy-shared-conflict-timing',
    passed: true,
    entry: lazyEntry,
    result: lazyFailure,
    logs: lazyBad.logs,
  });
  await lazyBad.page.close();
  ...
  for (const variant of ['good', 'duplicatesdk']) {
    ...
    function visit(item) {
      ...
      for (const child of [...(item.modules ?? []), ...(item.children ?? [])]) visit(child);
    }
    visit(stats);
    const privateSdk = names.filter(
      (name) => /plugin-runtime.*ui-react/.test(name) && !/consume shared/.test(name),
    );
    const privateReact = names.filter(
      (name) => /node_modules\/react\/(?:cjs|index)/.test(name) && !/consume shared/.test(name),
    );
    report.bundleAudit[variant] = { privateSdk, privateReact };
    if (variant === 'good') {
      assert.deepEqual(privateSdk, []);
      assert.deepEqual(privateReact, []);
    } else assert.ok(privateSdk.length > 0);
  }
  for (const [file, expected] of Object.entries(report.snapshots))
    assert.equal(
      digest(await readFile(join(repo, file))),
      expected,
      `production input changed: ${file}`,
    );
  ...
  console.error(error);
  report.failurePages = [];
  for (const { page, logs } of openedPages)
    if (!page.isClosed())
      report.failurePages.push({
        url: page.url(),
        logs,
        state: await page
          .evaluate(() => ({
            errors: window.poc?.errors,
            ui: window.poc?.ui?.core.inspect(),
  ...
  await writeFile(
    process.env.NEXUS_POC_REPORT ??
      join(repo, 'docs/validation/plugin-externalization-poc-results.json'),
