...
const runtime = 'packages/plugin-runtime',
  host = 'apps/host';
const results = [],
  changes = new Map();
...
function replace(path, before, after) {
  edit(path, (original) => {
    if (original.split(before).length !== 2)
      throw Error(`Expected unique anchor in ${path}: ${before}`);
    return original.replace(before, after);
  });
}
function command(pkg, binary, args, timeout = 60_000) {
  return spawnSync(process.execPath, [join(root, pkg, 'node_modules', binary), ...args], {
...
function test(name, pkg) {
  ...
  const child = command(pkg, 'vitest/vitest.mjs', [
    'run',
    '--config',
  ...
  const result = {
    ...
    observations: [
      ...[...logs.matchAll(/SUBSTITUTION_POLICY_CALLS\s+(\d+)/g)].map((m) => ({
        policyCalls: Number(m[1]),
      })),
      ...[...logs.matchAll(/SUBSTITUTION_ROUTE_CASES\s+(\d+)\s+DIFFERENCES\s+(\d+)/g)].map((m) => ({
        routeCases: Number(m[1]),
        differences: Number(m[2]),
      })),
    ],
    failedTests: report.testResults.flatMap((suite) =>
      suite.assertionResults
        .filter((t) => t.status === 'failed')
        .map((t) => ({ name: t.fullName, messages: t.failureMessages })),
    ),
    suiteErrors: report.testResults
      .filter((s) => s.assertionResults.length === 0 && s.status === 'failed')
      .map((s) => s.message),
  };
  if (!report.numTotalTests || result.suiteErrors.length || child.error)
    throw Error(JSON.stringify(result));
  results.push(result);
  console.log(JSON.stringify({ ...result, failedTests: result.failedTests.map((t) => t.name) }));
...
function capture(name) {
  ...
  for (const [path, original] of changes) {
    ...
    const before = join(scratch, 'diff/before', path),
      after = join(scratch, 'diff/after', path);
    const diff = spawnSync('git', ['diff', '--no-index', '--no-ext-diff', '--', before, after], {
      encoding: 'utf8',
    });
...
function reset() {
  for (const [path, original] of changes) writeFileSync(join(scratch, path), original);
  changes.clear();
}
...
try {
  ...
  writeFileSync(
    join(scratch, 'vitest.substitution.mjs'),
    `export default ${JSON.stringify({ resolve: { alias: aliases } })};`,
  );
  ...
  for (const file of ['route-model.ts', 'path-space.ts'])
    cpSync(join(scratch, host, 'src/routing', file), join(scratch, host, 'test/reference', file));
  writeFileSync(
    join(scratch, host, 'test/substitution.test.ts'),
    readFileSync(join(root, 'docs/validation/substitution-route-probes.txt'), 'utf8'),
  );
  for (const pkg of [runtime, host]) {
    if (
      !test(`baseline-${pkg.split('/').at(-1)}`, pkg) ||
      !typecheck(`baseline-types-${pkg.split('/').at(-1)}`, pkg)
    )
      throw Error('Baseline failed.');
  }
  ...
  edit(`${runtime}/src/surface-definition.ts`, (original) => {
    const recordTypes = original.slice(
      original.indexOf('export interface SurfaceDefinitionRecord'),
      original.indexOf('export interface SurfaceDefinitionActivation'),
    );
...
export function projectSurfaceDefinitions(registry: ContributionRegistry, versionOf: (owner: PluginId) => string | undefined): SurfaceDefinitionRegistry {
  ...
  for (const operation of ['validate', 'apply', 'discard'])
    replace(bootstrap, `      surfaceActivation.${operation}();\n`, '');
  replace(
    bootstrap,
  ...
  test('surface-projection-runtime', runtime);
  typecheck('surface-projection-types', runtime);
  ...
  replace(
    routeModel,
    'matchSpace(r.space, location.pathname)',
    'matchRoutes([{ path: r.space.path, caseSensitive: false }], location.pathname)',
  );
  capture('router-membership');
  test('router-membership-host', host);
  typecheck('router-membership-types', host);
  reset();
  ...
  edit(ui, (original) => {
    const start = original.indexOf('  function relations('),
      end = original.indexOf('  function endOccurrence(', start);
...
`;
  ...
  test('admission-at-initialization-runtime', runtime);
  typecheck('admission-at-initialization-types', runtime);
  reset();
  for (const name of ['surface-projection', 'router-membership', 'admission-at-initialization']) {
    const applied = spawnSync('git', ['apply', '--', join(evidence, `${name}.patch`)], {
      cwd: scratch,
      encoding: 'utf8',
    });
  ...
  writeFileSync(
    join(root, 'docs/validation/architecture-substitution-results.json'),
    JSON.stringify(
      {
        ...
        scope:
          'Independent replacements in temporary source tree; full package unit suites, added contract probes and typechecking; source aliases explicitly set; no browser E2E or performance claim.',
        results,
      },
      null,
      2,
