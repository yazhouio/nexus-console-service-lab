...
createServer(async (request, response) => {
  try {
    ...
    if (url.pathname.startsWith('/api/')) {
      const upstream = await fetch(`http://127.0.0.1:3002${url.pathname}${url.search}`, { headers: { cookie: request.headers.cookie ?? '' }, redirect: 'manual' });
    ...
    const reserved = /^\/(?:api|plugins|static|assets)(?:\/|$)/.test(url.pathname) || /\.(?:js|css|map|png|ico|svg|json)$/.test(url.pathname);
    if (!exists && !reserved && request.headers.accept?.includes('text/html')) { file = resolve(root, 'index.html'); exists = true; }
    if (!exists) { response.writeHead(404, { 'content-type': 'text/plain' }); response.end('Not found'); return; }
    response.writeHead(200, { 'content-type': contentTypes[extname(file)] ?? 'application/octet-stream', 'cache-control': 'public, max-age=0' });
    response.end(await readFile(file));
  } catch { response.writeHead(500, { 'content-type': 'text/plain' }); response.end('Preview request failed'); }
}).listen(Number(process.env.PREVIEW_PORT ?? 3200), '127.0.0.1', () => console.log('Routing validation preview ready'));
