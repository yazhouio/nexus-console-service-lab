...
const config = {
  ...
  configRevision: 'fallback-2',
  contributionContractVersion: 2,
  deepLinks: [
    {
      path: '/clusters/demo/nodes/n1',
      routeId: 'node-detail',
      params: { cluster: 'demo', node: 'n1' },
    },
  ],
  resources: ['api', 'plugin', 'javascript', 'stylesheet'].flatMap((kind) =>
    [false, true].map((missing) => ({
      kind,
      missing,
      path: `/${kind}/${missing}`,
      status: missing ? 404 : 200,
      contentType: kind === 'api' ? 'application/json' : 'text/plain',
    })),
  ),
  performancePaths: {
    initial: '/clusters/demo/nodes/n1',
    params: '/clusters/demo/nodes/n2',
    query: '/clusters/demo/nodes/n2?q=1',
    exit: '/',
    routeId: 'node-detail',
  },
...
const evidence = () => ({
  ...
  checks: [
    ...config.deepLinks.flatMap((d) =>
      ['open', 'refresh'].map((action) => ({ path: d.path, action, passed: true })),
    ),
  ...
  performance: { accepted: true, configDigest: configDigest(config) },
});
...
test('production routing is enabled by default while release evidence remains enforceable', () => {
  ...
  assert.throws(
    () => routingBuildSettings({ NODE_ENV: 'production', NEXUS_REQUIRE_ROUTING_EVIDENCE: 'true' }),
    /evidence|config/i,
  );
...
test('validation builds are explicitly separated from release output', () => {
  const settings = routingBuildSettings({
    NODE_ENV: 'production',
    NEXUS_ROUTING_VALIDATION: 'true',
  });
