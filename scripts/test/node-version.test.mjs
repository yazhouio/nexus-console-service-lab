...
test('Node gate accepts supported Node 24 releases and rejects other majors or older patches', () => {
  for (const version of ['24.14.1', '24.14.2', '24.20.0'])
    assert.equal(supportsNode(version), true, version);
  for (const version of ['24.14.0', '23.14.1', '25.0.0', 'garbage', '24.14.1-rc.1'])
...
test('CLI exits nonzero for an unsupported runtime and succeeds on the current supported runtime', async () => {
  ...
  const setup =
    ...
    encodeURIComponent('Object.defineProperty(process.versions, "node", {value: "25.0.0"});');
