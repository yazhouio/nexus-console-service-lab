...
async function compile(file, namespace = 'fixture') {
  const assets = new Map(), dependencies = [];
  const source = await new Promise((resolve, reject) => loader.call({
    resourcePath: file, getOptions: () => ({ namespace }),
    async: () => (error, code) => error ? reject(error) : resolve(code),
    emitFile(name, value) { assets.set(name, value); }, addDependency(file) { dependencies.push(file); },
  }));
...
test('artifact compilation bundles the complete CSS closure and publishes relative image URLs with content identities', async () => {
  ...
  try {
    ...
    await writeFile(join(directory, 'entry.css'), '@import "./nested/part.css"; .fixture-card { color: var(--nexus-color-text-primary); }');
    await writeFile(join(directory, 'nested/part.css'), '.fixture-picture { background-image: url("../picture.svg?v=1#icon"); }');
    ...
    await writeFile(join(directory, 'picture.svg'), '<svg xmlns="http://www.w3.org/2000/svg"><path/></svg>');
    const changed = await compile(join(directory, 'entry.css'));
    assert.notEqual([...changed.assets.keys()].find(name => name.endsWith('.css')), cssName);
...
test('CSS Modules retain JS mappings and use plugin identity plus source content in generated names', async () => {
  ...
  try {
    ...
    const first = await compile(file), independent = await compile(file, 'other');
...
test('build rejects incomplete imports, missing assets and invalid CSS', async () => {
  ...
  try {
    for (const source of ['@import "https://example.com/styles.css";', '@import "./missing.css";', '.fixture-card { background: url("missing.png") }', '.fixture-card { color: ']) {
      const file = join(directory, 'entry.css'); await writeFile(file, source);
...
test('compiled ownership rules reject resets, foreign classes, token writes and global registrations', () => {
  for (const css of [
    'button { color: red }', '.fixture-root button { color: red }', '.fixture-root [role=tab] { color: red }',
    '.foreign-button { color: red }', ':root { --nexus-color-surface: red }', '.fixture-root { --nexus-color-surface: red }',
    '@property --nexus-color-surface { syntax: "<color>"; inherits: false; initial-value: red }',
    '@keyframes spin { to { opacity: 0 } }', '@layer shared { .fixture-card { color: red } }',
    '@font-face { font-family: Shared; src: url(font.woff2) }', '.fixture-root { --other-value: red }',
    '.fixture-root { position: fixed }', '.fixture-root { z-index: 100 }', '.fixture-root { color: red!important }',
    '.fixture-root [data-nexus-slot] { display: none }', '.fixture-root:is(body) { color:red }',
  ]) assert.throws(() => checkCss(css, 'fixture'), css);
  assert.doesNotThrow(() => checkCss('.fixture-tab[aria-selected=true] { color:var(--nexus-color-action-primary) } @keyframes fixture-spin { to { opacity:0 } }', 'fixture'));
});
