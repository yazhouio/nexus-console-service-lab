...
test('the production build rejects asynchronous CSS omitted from Distribution and rejects Host side-effect loading', async () => {
  ...
  try {
    await writeFile(
      ...
      '.fixture-card { color: var(--nexus-color-text-primary) }',
    );
    await writeFile(
      join(directory, 'lazy.js'),
      'import classes from "./style.css?artifact"; console.log(classes);',
    );
    for (const [source, accepted] of [
      ...
      const build = await createRsbuild({
        ...
        rsbuildConfig: {
          logLevel: 'silent',
          mode: 'production',
          source: { entry: { index: './distribution.js' } },
          ...
          tools: {
            bundlerChain(chain, { CHAIN_ID }) {
              ...
              chain.module
                ...
                .use('reject')
                .loader(join(root, 'scripts/reject-unmanaged-css.cjs'));
            },
          },
        },
      });
      if (accepted) await build.build();
      else await assert.rejects(build.build(), /Rspack build failed/);
    }
  } finally {
