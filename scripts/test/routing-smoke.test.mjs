...
test('detects fallback rewrites even for a plugin artifact expecting HTML', () => {
  ...
  assert.doesNotThrow(() =>
    assertResourceResponse(expected, {
      status: 200,
      contentType: 'text/html; charset=utf-8',
      body: '<html><div>plugin</div></html>',
    }),
  );
  assert.throws(
    () =>
      assertResourceResponse(expected, {
        ...
        body: '<html><head><meta name="nexus-host" content="true"></head></html>',
      }),
    /rewritten/,
  );
  assert.throws(
    () =>
      assertResourceResponse(
        { kind: 'javascript', status: 404, contentType: 'text/plain' },
