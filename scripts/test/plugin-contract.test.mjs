...
test('original compiler rejects illegal bindings and expanded profiled declarations', () => {
  ...
  owner.points.find(p => p.id === 'node.actions').bindings.itemRefContract = 'cluster.missing-ref@1';
...
for (const field of ['ownerPluginId', 'id', 'contractMajor', 'kind', 'duplicate']) test(`Catalog rejects ${field} mismatch`, () => {
  const changed = copy();
...
test('coverage is display metadata; partial empty groups remain unknown and compiler output is unaffected', () => {
  ...
  assert.doesNotMatch(JSON.stringify(sources.map(s => [s.descriptor, s.points, s.contributions])), /coverage/);
});
...
test('CLI replaces only its dedicated directory and removes output on validation failure', async () => {
  ...
  try {
    for (const folder of ['scripts', 'packages', 'apps']) await cp(resolve(root, folder), resolve(directory, folder), {
      recursive: true, filter: path => !/(?:^|\/)(?:node_modules|dist|doc_build|\.rspress)(?:\/|$)/.test(path),
    });
    await writeFile(resolve(directory, 'package.json'), '{"type":"module"}');
    for (const pkg of ['packages/console-core', 'apps/console']) await symlink(resolve(root, pkg, 'node_modules'), resolve(directory, pkg, 'node_modules'));
    ...
    const run = () => execFileSync(process.execPath, ['scripts/generate-plugin-contract.mjs'], { cwd: directory, stdio: 'pipe' });
    run();
    ...
    await writeFile(source, (await readFile(source, 'utf8')).replace('[kubeeyeManifest, kubeeyeManifestV2]', '[kubeeyeManifest]'));
    run();
