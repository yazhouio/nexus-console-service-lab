...
try {
  const sx = transformSync(
    ...
    {
      filename: join(scratch, 'sx.ts'),
      configFile: false,
      babelrc: false,
      plugins: [[stylex, { dev: false, runtimeInjection: false, classNamePrefix: 'sx-' }]],
    },
  );
  const sxCss = stylex.processStylexRules(sx.metadata.stylex, {
    useLayers: { prefix: 'sx-layer' },
  });
  const tw = await compile(
    '@import "tailwindcss/utilities.css" layer(tw-utilities) source(none); @theme inline prefix(tw) { --color-body: var(--nexus-color-text-primary); --color-surface: var(--nexus-color-surface); --spacing: var(--nexus-space-1); }',
    ...
    rsbuildConfig: {
      ...
      tools: {
        bundlerChain(chain, { CHAIN_ID }) {
          ...
          for (const namespace of ['sx', 'tw'])
            chain.module
              .rule(namespace)
              .test(new RegExp(namespace + '\\.css$'))
              .resourceQuery(/artifact/)
              .type('javascript/auto')
              .use('artifact')
              .loader(join(root, 'scripts/artifact-css-loader.cjs'))
              .options({ namespace });
  ...
  for (const order of [
    ...
    await page.waitForFunction(() =>
      [...window.harness.mounts.values()].every((root) => root.execution.phase === 'ready'),
    );
    const read = () =>
      page.evaluate(() =>
        Object.fromEntries(
    ...
    assert.deepEqual(before.sx, before.tw);
    assert.equal(before.sx.padding, '16px');
    assert.equal(before.host.color, 'rgb(5, 6, 7)');
    const scope = await page.evaluate(() =>
      [...window.harness.mounts.values()].map((root) => root.attemptId),
    );
    await page.addStyleTag({
      content:
        ':root[data-nexus-theme="test"] { --nexus-color-text-primary: rgb(90, 30, 120); --nexus-color-surface: rgb(240, 235, 250); --nexus-space-4: 28px; --nexus-space-1: 7px; }',
    });
    await page.evaluate(() => {
      document.documentElement.dataset.nexusTheme = 'test';
    });
    const after = await read();
    assert.deepEqual(after.sx, after.tw);
    assert.equal(after.sx.color, 'rgb(90, 30, 120)');
    assert.equal(after.sx.padding, '28px');
    assert.deepEqual(after.host, before.host);
    assert.deepEqual(
      await page.evaluate(() => [...window.harness.mounts.values()].map((root) => root.attemptId)),
      scope,
    );
    await page.evaluate(() => {
      window.harness.block();
      window.harness.close('sx');
    });
    await page.waitForFunction(() => window.harness.trace.some((item) => item.event === 'before'));
    assert.equal(await page.locator('#sx').count(), 1);
    assert.equal(await page.locator('link[data-nexus-artifact-css]').count(), 2);
    await page.evaluate(() => {
      window.throwCleanup = true;
      window.harness.unblock();
    });
    await page.waitForFunction(() => !document.getElementById('sx'));
    await page.waitForFunction(
      () => document.querySelectorAll('link[data-nexus-artifact-css]').length === 1,
    );
    ...
    results.push({
      ...
      before,
      after,
      preservedAttempts: true,
      asyncThrowingCleanup: true,
      disposed: true,
    });
  }
  await mkdir(join(root, 'test-results'), { recursive: true });
  await writeFile(
    join(root, 'test-results/styling-compilers.json'),
    JSON.stringify({ passed: true, results }, null, 2),
  );
  console.log(
    'StyleX / Tailwind production compilation, order reversal, theme updates and asynchronous cleanup passed.',
  );
} finally {
  await browser?.close();
