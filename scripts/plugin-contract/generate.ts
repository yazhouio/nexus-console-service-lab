...
} from '../../packages/plugin-runtime/src/contribution.ts';
import {
  assertRouteMetadata,
  assertNavigationMetadata,
} from '../../packages/plugin-runtime/src/contribution.ts';
import {
  validateRestrictedInstallRecord,
  type RestrictedPluginManifest,
} from '../../packages/plugin-runtime/src/manifest.ts';
import {
...
} from '../../packages/plugin-runtime/src/ui/definitions.ts';
import {
  createPointCompiler,
  type PointContracts,
} from '../../packages/plugin-runtime/src/ui/point-compiler.ts';
...
export interface Source {
  ...
  readonly catalog?: readonly {
    ref: ExtensionPointRef;
    kind: ExtensionKind;
    title: string;
    description: string;
  }[];
  readonly contributions: {
    readonly routes?: readonly Omit<RouteContribution, 'target'>[];
    readonly navigation?: readonly NavigationContribution[];
    readonly extensions?: readonly ExtensionContributionDefinition[];
  };
  readonly coverage: Readonly<
    Record<'points' | 'routes' | 'navigation' | 'extensions', 'complete' | 'partial' | 'unknown'>
  >;
...
const notice =
  '> 本仓库选定声明的参考，非当前安装或可用状态。引用关系不代表授权、可见性或执行成功；实际结果请查看 Runtime Inspector。\n';
...
const filename = (source: Source) =>
  `plugin-${encodeURIComponent(source.descriptor.id)}-${encodeURIComponent(source.descriptor.version)}.md`;
function table(headers: string[], rows: string[][]) {
  return `| ${headers.join(' | ')} |\n| ${headers.map(() => '---').join(' | ')} |\n${rows.map((row) => `| ${row.map(escapeMarkdown).join(' | ')} |`).join('\n')}\n`;
}
function coverage(source: Source) {
  return (
    table(['声明分组', '收录范围'], Object.entries(source.coverage)) +
...
export function generatePages(
  ...
  for (const source of sources) {
    if (identities.has(identity(source)))
      throw Error(`Duplicate plugin version: ${identity(source)}`);
    identities.add(identity(source));
    if (Object.values(source.coverage).includes('complete') && !source.evidence.trim())
      throw Error('Complete coverage requires evidence');
    if (source.manifest) {
  ...
  const points = sources.flatMap((source) => {
    ...
    for (const annotation of source.catalog ?? []) {
      ...
      if (
        !entry ||
        ref.ownerPluginId !== source.descriptor.id ||
        ref.contractMajor !== entry.point.contractMajor ||
        annotation.kind !== entry.point.kind ||
        seen.has(ref.id)
      ) {
        throw Error(
          `Catalog mismatch or duplicate: ${identity(source)}/${ref.ownerPluginId}/${ref.id}@${ref.contractMajor} (${annotation.kind})`,
        );
      }
      seen.add(ref.id);
  ...
  const contributions = sources.flatMap((source) =>
    (['routes', 'navigation', 'extensions'] as const).flatMap((group) =>
      sorted<
        Omit<RouteContribution, 'target'> | NavigationContribution | ExtensionContributionDefinition
      >(source.contributions[group] ?? []).map((value) => {
        if (group === 'routes')
          assertRouteMetadata(source.descriptor.id, value as Omit<RouteContribution, 'target'>);
        else if (group === 'navigation')
          assertNavigationMetadata(source.descriptor.id, value as NavigationContribution);
        ...
        return {
          ...
          kind:
            group === 'routes'
              ...
              : group === 'navigation'
                ? 'navigation'
                : (value as ExtensionContributionDefinition).kind,
        };
      }),
    ),
  );
  for (let i = 0; i < contributions.length; i++) {
    ...
    for (const b of contributions.slice(i + 1)) {
      ...
      if (sameOwner || a.group !== 'extensions')
        throw Error(
          `Duplicate ${a.group} identity: ${identity(a.source)}/${a.value.id}, ${identity(b.source)}`,
        );
    }
  }
  function relation(
    source: Source,
    value: { point?: ExtensionPointRef; expectedProfile?: string; expectedRefContract?: string },
    kind: string,
  ) {
    ...
    const candidates = points.filter(
      (p) =>
        p.source.descriptor.id === ref.ownerPluginId &&
        p.point.id === ref.id &&
        (p.source.descriptor.id !== source.descriptor.id ||
          p.source.descriptor.version === source.descriptor.version),
    );
    ...
    return candidates
      .map((p) => {
        ...
        if (value.expectedProfile && value.expectedProfile !== p.point.profile)
          diagnostics.push('expectedProfile 不匹配');
        if (
          value.expectedRefContract &&
          !Object.values(p.compiled.refContracts).some((r) => r.id === value.expectedRefContract)
        )
          diagnostics.push('expectedRefContract 不匹配');
  ...
  for (const profile of sorted(contracts.profiles ?? []))
    reference += `\n### ${escapeMarkdown(profile.id)}\n\n${json(profile)}`;
  reference += '\n## Ref Contract\n\n领域 Schema、traits 和 predicates 来自原 RefContract。\n';
  for (const ref of sorted(contracts.refContracts ?? []))
  ...
  for (const { source, point, compiled, title, description } of points) {
    ...
    const refs = contributions.filter(
      (c) =>
        c.value.point?.ownerPluginId === source.descriptor.id &&
        c.value.point.id === point.id &&
        (c.source.descriptor.id !== source.descriptor.id ||
          c.source.descriptor.version === source.descriptor.version),
    );
    pointPage += refs.length
      ? table(
          ['插件版本 / 分组 / Contribution', '声明关系'],
          refs.map((c) => [
            `${identity(c.source)} / ${c.group} / ${c.value.id}`,
            relation(c.source, c.value, c.kind),
          ]),
        )
      : '所选来源中没有收录反向引用；不推断运行时零贡献。\n';
  }
  ...
  for (const source of sources) {
    ...
    page += '\n## Extension Points\n\n';
    for (const point of sorted(source.points))
      page += `### ${escapeMarkdown(point.id)}\n\n${json(point)}\n`;
    if (!source.points.length)
      page +=
        source.coverage.points === 'complete'
          ? '该版本未声明 Point。\n'
