import { deploymentDescriptor, deploymentRoutes, deploymentNavigation, deploymentExtensions, deploymentHpaDescriptor, deploymentVpaDescriptor, deploymentMonitoringDescriptor, deploymentNetworkDescriptor } from '../../apps/console/src/plugins/deployment-data.ts';
...
import { CLUSTER_EXTENSION_POINTS, CLUSTER_EXTENSION_POINT_CATALOG, RESOURCE_REF_CONTRACT, DEPLOYMENT_EXTENSION_POINTS, DEPLOYMENT_PROFILES } from '../../packages/cluster-api/src/index.ts';
...
export const contracts = { profiles: [...CONSOLE_PROFILES, ...DEPLOYMENT_PROFILES], refContracts: [PLUGIN_REF_CONTRACT, RESOURCE_REF_CONTRACT] };
...
export const sources: readonly Source[] = [
  ...
  { descriptor: deploymentDescriptor, points: DEPLOYMENT_EXTENSION_POINTS,
    contributions: { routes: Object.values(deploymentRoutes), navigation: [deploymentNavigation], extensions: [] },
    coverage: complete, evidence: 'deployment.tsx 使用 deployment-data.ts 并遍历 DEPLOYMENT_EXTENSION_POINTS。', origin: 'apps/console/src/plugins/deployment-data.ts' },
  ...[
  ...
  ].map(({ descriptor, ids }) => ({ descriptor, points: [], contributions: { routes: [], navigation: [], extensions: ids.map(id => deploymentExtensions[id]) }, coverage: complete,
    evidence: 'deployment-extensions.tsx 使用 deployment-data.ts 中对应插件的固定贡献；无路由、导航或扩展点注册。', origin: 'apps/console/src/plugins/deployment-data.ts' })),
