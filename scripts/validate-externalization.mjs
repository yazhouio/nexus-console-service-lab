...
const host = join(scratch, 'host'),
  ...
  restricted = join(scratch, 'restricted'),
  tarballs = join(scratch, 'tarballs');
await Promise.all(
  [host, remote, restricted, tarballs].map((path) => mkdir(path, { recursive: true })),
);
...
async function run(cmd, args, cwd = repo) {
  await new Promise((resolve, reject) => {
    ...
    child.on('close', (code) =>
      code ? reject(Error(`${cmd} ${args.join(' ')}\n${output}`)) : resolve(output),
    );
  });
}
const servers = [];
...
async function serve(dir, remoteServer = false) {
  const server = createServer(async (req, res) => {
    ...
    const file = remoteServer
      ? path.replace(/^\/extension-demo\/1\.0\.0\//, '')
      : extname(path)
        ? path
        : 'index.html';
    ...
    try {
      ...
      const bytes = await readFile(
        restrictedPath
          ? join(restricted, 'dist', path.slice('/plugins/kubeeye/1.0.0/'.length) || 'index.html')
          : join(dir, file),
      );
      res.setHeader(
        'Content-Type',
      ...
      report.requests.push({ remote: remoteServer, path, status: 200, bytes: bytes.length });
      res.end(bytes);
    } catch {
      report.requests.push({ remote: remoteServer, path, status: 404 });
...
try {
  ...
  for (const name of packages) {
    ...
    for (const group of ['dependencies', 'peerDependencies', 'optionalDependencies'])
      for (const version of Object.values(packed[group] ?? {}))
        assert.ok(!/^(workspace|catalog):/.test(version), `${name}: unresolved ${version}`);
  }
  const common = {
    ...deps,
    react: '19.2.8',
  ...
  for (const file of ['rsbuild.config.ts', 'tsconfig.json'])
    await cp(join(repo, 'apps/example-builtin-plugin', file), join(remote, file));
  await writeFile(
    join(remote, 'package.json'),
    JSON.stringify({
      name: 'external-demo',
      overrides: { '@rspack/core': '2.2.2' },
      version: '1.0.0',
      type: 'module',
      dependencies: { ...common, '@module-federation/runtime-tools': '2.9.0' },
    }),
  );
  await writeFile(
  ...
  await cp(join(repo, 'apps/example-restricted-plugin/src'), join(restricted, 'src'), {
    recursive: true,
  });
  for (const file of [
    'index.html',
    ...
    await cp(join(repo, 'apps/example-restricted-plugin', file), join(restricted, file));
  await writeFile(
  ...
  const installed = await Promise.allSettled(
    [host, remote, restricted].map((dir) =>
  ...
  for (const file of ['cluster.tsx', 'cluster-data.ts', 'cluster.css', 'kubeeye-installation.ts'])
    await cp(join(repo, 'apps/console/src/plugins', file), join(host, file));
  for (const file of ['manifest.json', 'manifest-v2.json'])
    await cp(join(restricted, file), join(host, file));
  await writeFile(
  ...
  await writeFile(
    join(remote, '.env'),
    `NEXUS_REMOTE_BASE=${remoteOrigin}/extension-demo/1.0.0/\n`,
  );
  await cp(
    join(repo, 'apps/console/src/federation-builtins.ts'),
    join(host, 'federation-builtins.ts'),
  );
  await cp(join(repo, 'apps/console/src/routing/contribution-policy.ts'), join(host, 'policy.ts'));
  await writeFile(
    join(host, 'env.d.ts'),
    "declare module '*.css'; declare module '*.css?artifact' { export const css: readonly string[]; }\n",
  );
  await writeFile(
...
`,
  ...
  for (const dir of [remote, host]) {
    await run('npx', ['tsc', '--noEmit'], dir);
    await run('npx', ['rsbuild', 'build'], dir);
  }
  ...
  await page
    .getByRole('button', { name: /Copy console-core/ })
    .first()
    .click();
  ...
  const scriptCount = report.requests.filter(
    (request) => request.remote && request.path.endsWith('.js'),
  ).length;
  ...
  assert.equal(
    report.requests.filter((request) => request.remote && request.path.endsWith('.js')).length,
    scriptCount,
  );
  ...
  await page
    .locator('pre')
    .filter({ hasText: /"checkedBy":\s*"extension-demo"/ })
    .waitFor();
  ...
  for (const [label, directory] of [
    ...
    try {
      ...
      report.versions[label] = Object.fromEntries(
        [
          '@rsbuild/core',
          '@rspack/core',
      ...
      /* Installation may have failed before generating a lockfile. */
    }
  }
  await writeFile(
    join(repo, 'test-results/externalization/results.json'),
    ...
    JSON.stringify(
      {
        ...
        cases: report.cases,
        routeReadyMs: report.routeReadyMs,
        warmRouteReadyMs: report.warmRouteReadyMs,
        error: report.error,
        evidence: 'test-results/externalization/results.json',
      },
      null,
      2,
    ),
